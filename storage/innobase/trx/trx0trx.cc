...
static void trx_init(trx_t *trx) {
  ...
  trx->updated_max_cts = 0;

  trx->is_rollback_finished = false;

  trx->in_setting_cts = false;
    ...
    if (undo->state == TRX_UNDO_PREPARED) {
      ...
      if (srv_force_recovery == 0) {
        ...
        if (!trx_is_mysql_xa(trx)) {
			trx_sys_update_trx_status(trx, TRX_SYS_STATUS_PREPARED);
		}
        ...
        trx_sys_update_trx_status(trx, TRX_SYS_STATUS_RUNNING);
      }
    } else {
...
//  } else {
//	  trx_sys_get_new_trx_id_with_thd(trx);
//  }
//
//
//  if (trx->preallocated_id) {
...
//  }
//}
//>>>>>>> 10b2e1bf38b... #WARP-60738 maintain trx status or cts cache
    ...
    bool read_write) /*!< in: true if read-write transaction */
{
  ...
  if (!trx->read_only &&
    ...
    if (thd != nullptr && thd->tx_sts != 0) {
	   trx_sys_update_trx_status(trx, TRX_SYS_STATUS_RUNNING, thd->tx_sts);
    } else {
	   trx_sys_update_trx_status(trx, TRX_SYS_STATUS_RUNNING);
    }
...
static void trx_release_impl_and_expl_locks(trx_t *trx, bool serialized) {
  ...
  if (trx->id > 0) {
    /* When trx ROLLBACK or COMMIT, clean trx_sys->status_cache and trx_sys->evicted_status_map */
    trx_sys_clean_trx_status(trx->id);
...
void trx_commit_low(trx_t *trx, mtr_t *mtr)
{
  ...
  if (trx->is_recovered) {
	if (trx->cts > 0) {
	  trx_sys_update_max_ts(trx->cts);
	} else {
	  // set default transaction cts
	  trx->cts = trx_sys_get_max_ts() + 1;
	  // make sure gxid in the same row in ascending order
	  if (trx->cts < trx->updated_max_cts + 1) {
	    trx->cts = trx->updated_max_cts + 1;
	  }
	}
  } else {
	// set default transaction cts
	...
	THD *thd = trx->mysql_thd;
	if (thd != nullptr && thd->tx_cts > 0) {
	  trx_sys_update_max_ts(thd->tx_cts);
	  trx->cts = thd->tx_cts;
	}
  }

  // update trx status cache
  if (!trx->is_rollback_finished) {
	  trx_sys_update_trx_status(trx, trx->cts);
  }

  // set cts into all the rows modified by this trx before commit
  trx_set_cts_low(trx);
...
/** set xid before preparing a transaction for given rollback segment.
 @return lsn_t: lsn assigned for commit of scheduled rollback segment */
static lsn_t trx_set_undo_xid_for_prepare_low(
    trx_t *trx,               /*!< in/out: transaction */
    trx_undo_ptr_t *undo_ptr, /*!< in/out: pointer to rollback
                              segment scheduled for prepare. */
    bool noredo_logging)      /*!< in: turn-off redo logging. */
{
  if (undo_ptr->insert_undo != nullptr || undo_ptr->update_undo != nullptr) {
    ...
    if (undo_ptr->insert_undo != nullptr) {
      /* It is not necessary to obtain trx->undo_mutex here
      because only a single OS thread is allowed to do the
      xid setting for this transaction. */
      trx_undo_set_xid(trx, undo_ptr->insert_undo, &mtr);
    }

    if (undo_ptr->update_undo != nullptr) {
      trx_undo_set_xid(trx, undo_ptr->update_undo, &mtr);
    }
...
/**
Set undo xid before Prepares a transaction. */
static
void
trx_set_undo_xid_for_prepare(
                             /*========*/
                             trx_t*	trx)
{
  /* This transaction has crossed the point of no return and cannot
	...
	if (trx->rsegs.m_redo.rseg != NULL && trx_is_redo_rseg_updated(trx)) {
		...
		trx_set_undo_xid_for_prepare_low(trx, &trx->rsegs.m_redo, false);
	}
	...
	if (trx->rsegs.m_noredo.rseg != NULL
		...
		trx_set_undo_xid_for_prepare_low(trx, &trx->rsegs.m_noredo, true);
	}
...
//  ut_a(!trx->is_recovered);
  ...
  trx_sys_mutex_enter();
  trx->state = TRX_STATE_PREPARED;
  if (!trx_is_mysql_xa(trx) && ( trx->is_recovered || trx->mysql_thd == nullptr
				  || !trx->mysql_thd->is_one_phase_commit() )) {
	  trx_sys_update_trx_status(trx, TRX_SYS_STATUS_PREPARED);
  }
  trx_sys->n_prepared_trx++;
...
/**
Set the transaction xid into the undo log before prepare for MySQL.
@param[in, out] trx		Transaction instance to prepare */
dberr_t
trx_set_undo_xid_for_prepare_for_mysql(trx_t* trx)
{
	trx_start_if_not_started_xa(trx, false);
	...
	trx->op_info = "set_undo_xid_for_prepare";

	trx_set_undo_xid_for_prepare(trx);
    ...
    MEM_ROOT *mem_root)       /*!< in: memory for table names */
{
  ...
  for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != nullptr;
    ...
    trx->is_recovered.
    All user xa transaction should be record in xid_list, which will be handled in xarecover_handlerton in server layer.*/
    if (trx_state_eq(trx, TRX_STATE_PREPARED) || (trx->xid && !trx->xid->get_my_xid() && trx->xid->get_gtrid_length() > 0)) {
      if(trx_state_eq(trx, TRX_STATE_PREPARED)) {
                    ...
                    identifier */
{
  ...
  for (trx = UT_LIST_GET_FIRST(trx_sys->rw_trx_list); trx != nullptr;
    ...
    if (trx->is_recovered
        ...
        && xid->eq(trx->xid)) {
        /*This transaction is failed during xa prepare, so its state is
         * TRX_STATE_ACTIVE, and should be recover to be prepared.*/
    	break;
    }
...
void trx_set_rw_mode(trx_t *trx) /*!< in/out: transaction that is RW */
{
  ...
  if (MVCC::is_view_active(trx->read_view)) {
    MVCC::set_view_creator_trx_id(trx->read_view, trx->id, trx->cts);
  }
