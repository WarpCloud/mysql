...
/** Parses the row reference and other info in a fresh insert undo record.
@param[in,out]	node	row cts node
@param[in]      thd     THD associated with the node
@param[in,out]	mdl	MDL ticket or nullptr if unnecessary */
static void row_cts_ins_parse_undo_rec(cts_node_t *node, THD *thd,
                                        MDL_ticket **mdl) {
  dict_index_t *clust_index;
  byte *ptr;
  undo_no_t undo_no;
  ...
  /* Skip the UNDO if we can't find the table or the .ibd file. */
  if (node->table == nullptr) {
  } else if (node->table->ibd_file_missing || !node->table->is_visible_user_table()) {
  close_table:
    dd_table_close(node->table, thd, mdl, false);
    ...
    if (clust_index != nullptr) {
      ...
      trx_cts_update_rec_get_update(ptr, clust_index, node->trx, node->heap,
                                             &(node->update));
...
/** Update cts into a fresh insert of a row to a table. A fresh insert means that
 the same clustered index unique key did not have any record, even delete
 marked, at the time of the insert.
 @return DB_SUCCESS or DB_OUT_OF_FILE_SPACE */
dberr_t row_cts_ins(cts_node_t *node, /*!< in: row cts node */
                     que_thr_t *thr)    /*!< in: query thread */
{
  dberr_t err;
  MDL_ticket *mdl = nullptr;
  ...
  row_cts_ins_parse_undo_rec(node, thd,
                              dd_mdl_for_undo(node->trx) ? &mdl : nullptr);
