...
/** Set xid of the undo log segment before "xa prepare"/"commit one phase".
@param[in,out]	trx		Transaction
@param[in,out]	undo		Insert_undo or update_undo log
@param[in,out]	mtr		Mini-transaction
@return undo log segment header page, x-latched */
page_t *trx_undo_set_xid(trx_t *trx, trx_undo_t *undo, mtr_t *mtr);
...
#define TRX_UNDO_FLAG_GXID                   \
  0x80 /*!< TRUE if undo log header includes \
       cts */
...
/** cts offset*/
#define TRX_UNDO_TRX_GXID (TRX_UNDO_XA_XID + XIDDATASIZE)
/*--------------------------------------------------------------*/
#define TRX_UNDO_LOG_XA_HDR_SIZE (TRX_UNDO_TRX_GXID + 8)
/*!< Total size of the undo log header
with the XA XID and TRX_GXID */
/** @} */
