...
/** Table is visible user table. */
#define DICT_TF2_VISIBLE_USER_TABLE 2048
...
#define DICT_TABLE_MAGIC_N 76333786
  ...
  uint16_t get_n_sys_cols() const {
    ut_ad(magic_n == DICT_TABLE_MAGIC_N);
    uint16_t n_sys_cols = DATA_N_SYS_COLS;
    if (is_intrinsic()) {
    	n_sys_cols = DATA_ITT_N_SYS_COLS;
    } else if (is_visible_user_table()) {
    	n_sys_cols = DATA_N_SYS_COLS + 1;
    }
  ...
  bool is_visible_user_table() const {
     if (flags2 & DICT_TF2_VISIBLE_USER_TABLE) {
       return (true);
     }
