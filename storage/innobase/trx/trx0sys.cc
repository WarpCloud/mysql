...
/** For statistics on trx status cache and blocking read */
// Statistics for four kind of types in trx_sys->status_cache
std::atomic<int> running_in_trx_sys_status_cache{0};
std::atomic<int> prepared_in_trx_sys_status_cache{0};
std::atomic<int> rollback_in_trx_sys_status_cache{0};
std::atomic<int> cts_in_trx_sys_status_cache{0};

// Statistics for four kind of types in trx_sys->evicted_status_map
std::atomic<int> running_in_evicted_status_map{0};
std::atomic<int> prepared_in_evicted_status_map{0};
std::atomic<int> rollback_in_evicted_status_map{0};
std::atomic<int> cts_in_evicted_status_map{0};

// Recored the historical maximum value of size of trx_sys->evicted_status_map
uint64 max_size_evicted_status_map{0};

// Statistics for blocking read related
// Recored number of current blocking read trx
std::atomic<unsigned long long> blocking_read_trx_current{0};
// Recored number of times of total blocking read success and timeout trx
std::atomic<unsigned long long> blocking_read_trx_total{0};
// Recored number of times of total blocking read trx without timeout
std::atomic<unsigned long long> blocking_read_trx_total_without_timeout{0};
// Recored number of total blocking read time
std::atomic<unsigned long long> blocking_read_time_total{0};
// Recored number of total blocking read time without timeout part
std::atomic<unsigned long long> blocking_read_time_total_without_timeout{0};
// Recored number of times of total blocking read timeout
std::atomic<unsigned long long> blocking_read_timeout_total{0};
...
void trx_sys_status_cache_switch_status_add(trx_id_t status_or_cts) {
  switch(status_or_cts) {
    case TRX_SYS_STATUS_RUNNING:
      running_in_trx_sys_status_cache++;
      break;
    case TRX_SYS_STATUS_PREPARED:
      prepared_in_trx_sys_status_cache++;
      break;
    case TRX_SYS_STATUS_ROLLBACK:
      rollback_in_trx_sys_status_cache++;
      break;
    case TRX_SYS_STATUS_NONE:
      break;
    default:
      cts_in_trx_sys_status_cache++;
      break;
  }
}

void trx_sys_status_cache_switch_status_reduce(trx_id_t status_or_cts) {
  switch(status_or_cts) {
    case TRX_SYS_STATUS_RUNNING:
      running_in_trx_sys_status_cache--;
      break;
...
void trx_sys_evicted_status_map_switch_status_add(trx_id_t status_or_cts) {
  switch(status_or_cts) {
    case TRX_SYS_STATUS_RUNNING:
      running_in_evicted_status_map++;
      break;
    case TRX_SYS_STATUS_PREPARED:
      prepared_in_evicted_status_map++;
      break;
    case TRX_SYS_STATUS_ROLLBACK:
      rollback_in_evicted_status_map++;
      break;
    case TRX_SYS_STATUS_NONE:
      break;
    default:
      cts_in_evicted_status_map++;
      break;
  }
}

void trx_sys_evicted_status_map_switch_status_reduce(trx_id_t status_or_cts) {
  switch(status_or_cts) {
    case TRX_SYS_STATUS_RUNNING:
      running_in_evicted_status_map--;
      break;
...
void trx_sys_clean_trx_status(const trx_id_t id) {
	ut_ad(id > 0);
	int index = id % TRX_SYS_STATUS_CACHE_SIZE;
	trx_status_t existed_status(0, 0);
	bool found;
	do {
		found = false;
		existed_status = trx_sys->status_cache[index].load();
		if (existed_status.id != 0 && existed_status.id == id) {
			found = true;
			continue;
		}
		break;
	}while(!trx_sys->status_cache[index].compare_exchange_weak(existed_status, trx_status_t(0, 0)));

	if (found) {
                trx_sys_status_cache_switch_status_reduce(existed_status.status_or_cts);
		return;
	}

	trx_sys_status_mutex_enter();
	if (trx_sys->evicted_status_map.count(id) > 0) {
          trx_sys_evicted_status_map_switch_status_reduce(
            trx_sys->evicted_status_map[id]->status_or_cts);
		UT_DELETE(trx_sys->evicted_status_map[id]);
		trx_sys->evicted_status_map[id] = nullptr;
		trx_sys->evicted_status_map.erase(id);
	}
	trx_sys_status_mutex_exit();
}

// only record the rw_trxs
void trx_sys_update_trx_status(const trx_t *trx, const trx_id_t status_or_cts){
	trx_sys_update_trx_status(trx, status_or_cts, 0);
}

// only record the rw_trxs
void trx_sys_update_trx_status(const trx_t *trx, const trx_id_t status_or_cts, const trx_id_t sts){
	if (trx == nullptr || trx->internal || (status_or_cts > TRX_SYS_STATUS_RUNNING && !trx_is_rseg_updated(trx))) {
		return;
	}

	THD *thd = trx->mysql_thd;
	//only record non_system_thread transaction status
	if (thd != nullptr && thd->system_thread != NON_SYSTEM_THREAD) {
		return;
	}
	...
	int index = id % TRX_SYS_STATUS_CACHE_SIZE;
	trx_status_t existed_status(0, 0);
	trx_id_t updated_sts = sts;
	do {
		existed_status = trx_sys->status_cache[index].load();
		if (existed_status.id != 0) {

		    if(existed_status.id != id) {
		    	if (existed_status.id > id) {
		    		trx_sys_status_mutex_enter();
		    		if (trx_sys->evicted_status_map.count(id) > 0) {
		    			if (updated_sts > 0) {
		    				trx_sys->evicted_status_map[id]->sts = updated_sts;
		    			}
                                        trx_sys_evicted_status_map_switch_status_reduce(
                                          trx_sys->evicted_status_map[id]->status_or_cts);
		    			trx_sys->evicted_status_map[id]->status_or_cts = status_or_cts;
                                        trx_sys_evicted_status_map_switch_status_add(status_or_cts);
		    		} else {
		    			trx_sys->evicted_status_map[id] = UT_NEW_NOKEY(trx_status_sts_t(status_or_cts, sts));
                                        trx_sys_evicted_status_map_switch_status_add(status_or_cts);
                                        max_size_evicted_status_map =
                                          trx_sys->evicted_status_map.size() > max_size_evicted_status_map ?
                                          trx_sys->evicted_status_map.size() : max_size_evicted_status_map;
		    		}

		    		trx_sys_status_mutex_exit();
		    		// when the transaction's status is rollback or commited, it should be pushed into status queue to purge.
		    		if (status_or_cts > TRX_SYS_STATUS_PREPARED) {
		    			//TODO change unique mutex?
						trx_sys_wake_trx_commit(id);
		    		}
		    		return;
		    	}

		    	trx_sys_status_mutex_enter();
                        trx_sys_status_cache_switch_status_reduce(trx_sys->evicted_status_map[existed_status.id]->status_or_cts);
		    	trx_sys->evicted_status_map[existed_status.id] = UT_NEW_NOKEY(trx_status_sts_t(existed_status.status_or_cts, existed_status.sts));
                        trx_sys_evicted_status_map_switch_status_add(existed_status.status_or_cts);
                        max_size_evicted_status_map =
                          trx_sys->evicted_status_map.size() > max_size_evicted_status_map ?
                          trx_sys->evicted_status_map.size() : max_size_evicted_status_map;
                        trx_sys_status_mutex_exit();
		    } else {
		    	// sts would be put in only the first time, the updates should keep the sts not change.
                        trx_sys_status_cache_switch_status_reduce(existed_status.status_or_cts);
		    	if (updated_sts == 0) {
		    		updated_sts = existed_status.sts;
		    	}
		    }
		}

	}while(!trx_sys->status_cache[index].compare_exchange_weak(existed_status, trx_status_t(id, status_or_cts, updated_sts)));

          trx_sys_status_cache_switch_status_add(status_or_cts);
...
bool trx_sys_get_trx_status(const trx_id_t id, trx_id_t *status_or_cts, trx_id_t *sts) {
	trx_status_t trx_status = trx_sys->status_cache[id % TRX_SYS_STATUS_CACHE_SIZE].load();

	if (trx_status.id == id) {
		*status_or_cts = trx_status.status_or_cts;
		*sts = trx_status.sts;
		return true;
	}
	...
	trx_sys_status_mutex_enter();
	if (trx_sys->evicted_status_map.count(id) > 0) {
		*status_or_cts = trx_sys->evicted_status_map[id]->status_or_cts;
		*sts = trx_sys->evicted_status_map[id]->sts;
		trx_sys_status_mutex_exit();
		return true;
	}
	trx_sys_status_mutex_exit();
...
trx_id_t trx_sys_get_trx_status(const trx_id_t id) {
	trx_id_t status_or_cts=TRX_SYS_STATUS_NONE;
	trx_id_t sts=0;
	trx_sys_get_trx_status(id, &status_or_cts, &sts);
	return status_or_cts;
}

/** Waiting that the prepared transaction transformed.
 @param[in]   id   trx id to wait for
 @param[in]   thr  the current thr may be suspended
 @return DB status, success or blocking read timeout */
dberr_t trx_sys_wait_trx_commit(trx_id_t id, que_thr_t *thr){
  trx_id_t status_or_cts;
  ulint error;

  // create an event or reset an event
//  if (thr->blocking_read_wait_event == nullptr) {
//    thr->blocking_read_wait_event = os_event_create();
//    ut_a(thr->blocking_read_wait_event);
//  }
//  os_event_reset(thr->blocking_read_wait_event);

  mutex_enter(&trx_sys->trx_wait_event_mutex);
  status_or_cts = trx_sys_get_trx_status(id);
  if (status_or_cts > TRX_SYS_STATUS_PREPARED || status_or_cts == TRX_SYS_STATUS_NONE) {
    mutex_exit(&trx_sys->trx_wait_event_mutex);
    return DB_SUCCESS;
  }

  thr->blocking_read_wait_event = os_event_create();

  blocking_read_trx_current++;
  struct timespec ts_start, ts_end;
  clock_gettime(CLOCK_MONOTONIC, &ts_start);
  if (trx_sys->trx_wait_event_map.find(id) == trx_sys->trx_wait_event_map.end()) {
    trx_sys->trx_wait_event_map[id] = UT_NEW_NOKEY(std::vector<que_thr_t*>());
    trx_sys->trx_wait_event_map[id]->push_back(thr);
  } else if (std::find(trx_sys->trx_wait_event_map[id]->begin(),
                       trx_sys->trx_wait_event_map[id]->end(),
                       thr) == trx_sys->trx_wait_event_map[id]->end()){
    trx_sys->trx_wait_event_map[id]->push_back(thr);
  }
  mutex_exit(&trx_sys->trx_wait_event_mutex);
  //suspend until wake_up called; When wake_up has called, then return directly
  // limited time waiting, in microsecond
  error = os_event_wait_time_low(thr->blocking_read_wait_event,
                                 srv_blocking_read_wait_timeout * 1e3, 0);
  if (error != OS_SYNC_TIME_EXCEEDED) {

    clock_gettime(CLOCK_MONOTONIC, &ts_end);
    long delta = (ts_end.tv_sec - ts_start.tv_sec) * 1e9 + (ts_end.tv_nsec - ts_start.tv_nsec);
    blocking_read_time_total.fetch_add(delta);
    blocking_read_time_total_without_timeout.fetch_add(delta);
    blocking_read_trx_total_without_timeout++;
    blocking_read_trx_current--;
    blocking_read_trx_total++;

    os_event_destroy(thr->blocking_read_wait_event);
    return DB_SUCCESS;
  }

  blocking_read_time_total.fetch_add(srv_blocking_read_wait_timeout * 1e6);
  blocking_read_timeout_total++;
  blocking_read_trx_current--;
  blocking_read_trx_total++;

  os_event_destroy(thr->blocking_read_wait_event);
  return DB_BLOCKING_READ_WAIT_TIMEOUT;

}

/** Wake up the threads waiting for the trx */
void trx_sys_wake_trx_commit(trx_id_t id){
  mutex_enter(&trx_sys->trx_wait_event_mutex);
  if (trx_sys->trx_wait_event_map.find(id) == trx_sys->trx_wait_event_map.end()) {
	mutex_exit(&trx_sys->trx_wait_event_mutex);
	return;
  ...
  std::vector<que_thr_t*> *thr_items = trx_sys->trx_wait_event_map[id];
  if (thr_items->empty()){
    mutex_exit(&trx_sys->trx_wait_event_mutex);
    return;
  }

  for (que_thr_t* thr_item : *thr_items){
    //check how many threads are in  vector
    //int i = thr_item->blocking_read_wait_event == nullptr ? 1 : 0;
    //sql_print_warning("thr_item->blocking_read_wait_event is: %d", i);
    if(thr_item->blocking_read_wait_event != nullptr)
      os_event_set(thr_item->blocking_read_wait_event);
  }

  UT_DELETE(trx_sys->trx_wait_event_map[id]);
  trx_sys->trx_wait_event_map[id] = nullptr;
  trx_sys->trx_wait_event_map.erase(id);
...
void trx_sys_create(void) {
  ...
  mutex_create(LATCH_ID_TRX_SYS, &trx_sys->trx_status_mutex);

  for (int i = 0; i< TRX_SYS_STATUS_CACHE_SIZE; i++) {
	  trx_sys->status_cache[i] = trx_status_t(0, 0);
  }

  running_in_trx_sys_status_cache.store(0);
  prepared_in_trx_sys_status_cache.store(0);
  rollback_in_trx_sys_status_cache.store(0);
  cts_in_trx_sys_status_cache.store(0);

  trx_sys->evicted_status_map = std::unordered_map<trx_id_t, trx_status_sts_t*>();

  running_in_evicted_status_map.store(0);
  prepared_in_evicted_status_map.store(0);
  rollback_in_evicted_status_map.store(0);
  cts_in_evicted_status_map.store(0);

  trx_sys->max_ts = TRX_SYS_MAX_TS_INIT;

  trx_sys->trx_wait_event_map = std::map<trx_id_t, std::vector<que_thr_t*>*>();

  mutex_create(LATCH_ID_TRX_SYS, &trx_sys->trx_wait_event_mutex);
}
...
void trx_sys_close(void) {
  ...
  mutex_free(&trx_sys->trx_status_mutex);

  mutex_free(&trx_sys->trx_wait_event_mutex);

  trx_sys->rw_trx_ids.~trx_ids_t();

  trx_sys->rw_trx_set.~TrxIdSet();

  if (trx_sys->evicted_status_map.size() > 0) {
	  auto it = trx_sys->evicted_status_map.begin();
	  while(it != trx_sys->evicted_status_map.end()) {
		  UT_DELETE(it->second);
		  it->second = nullptr;
		  it = trx_sys->evicted_status_map.erase(it);
	  }
  }

  trx_sys->evicted_status_map.clear();
  ...
  if (trx_sys->trx_wait_event_map.size() > 0) {
	auto it = trx_sys->trx_wait_event_map.begin();
	while(it != trx_sys->trx_wait_event_map.end()) {
	  UT_DELETE(it->second);
	  it->second = nullptr;
	  it = trx_sys->trx_wait_event_map.erase(it);
	}
  }

  trx_sys->trx_wait_event_map.clear();
