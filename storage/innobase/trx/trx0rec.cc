...
byte *trx_undo_rec_get_pars(
	...
	uint8_t *undo_rec_flags)  /*!< out: undo record flags */
{
  const byte *ptr;
    ...
    mtr_t *mtr)           /*!< in: mtr */
{
  ...
  if (trx_undo_left(undo_page, ptr) < 61) {
  ...
  *ptr++ = (byte)rec_get_info_bits(rec, dict_table_is_comp(table));
  ...
  if (index->table->is_visible_user_table()) {
	  field = rec_get_nth_field(rec, offsets, index->get_sys_col_pos(DATA_GTRX_ID),
	                             &flen);
	   ut_ad(flen == DATA_GTRX_ID_LEN);

	   gtrx_id = trx_read_cts(field);

	   if (trx->updated_max_cts < gtrx_id) {
		   trx->updated_max_cts = gtrx_id;
	   }

	   ptr += mach_u64_write_much_compressed(ptr, gtrx_id);

	   *new_flag_ptr |= TRX_UNDO_REC_FLAG_GXID;
  }
...
byte *trx_undo_update_rec_get_sys_cols(
    const byte *ptr,      /*!< in: remaining part of undo
                          ...
                          general parameters */
	uint8_t undo_rec_flags, /*!< in: undo record flags*/
    trx_id_t *trx_id,     /*!< out: trx id */
    roll_ptr_t *roll_ptr, /*!< out: roll ptr */
	trx_id_t *gtrx_id,    /*!< out: gtrx id */
    ulint *info_bits)     /*!< out: info bits state */
{
  ...
  *roll_ptr = mach_u64_read_next_compressed(&ptr);
  if (undo_rec_flags & TRX_UNDO_REC_FLAG_GXID) {
	  *gtrx_id = mach_read_next_much_compressed(&ptr);
  }
    ...
    type_cmpl_t &type_cmpl)     /*!< out: type compilation info */
{
  ...
  update = upd_create(n_fields + n_system_fields, heap);
  ...
  if (undo_rec_flags & TRX_UNDO_REC_FLAG_GXID) {
	  /* Store gtrx id to update vector */
	  upd_field = upd_get_nth_field(update, n_fields + 2);

	  buf = static_cast<byte *>(mem_heap_alloc(heap, DATA_GTRX_ID_LEN));

	  trx_write_cts(buf, gtrx_id);

	  upd_field_set_field_no(upd_field, index->get_sys_col_pos(DATA_GTRX_ID), index,
							   trx);
	  dfield_set_data(&(upd_field->new_val), buf, DATA_GTRX_ID_LEN);
  }
  ...
  if (n_skip_field > 0) {
    ...
    upd_t *new_update = upd_create(n_fields + n_system_fields - n_skip_field, heap);

    for (i = 0; i < n_fields + n_system_fields; i++) {
      upd_field = upd_get_nth_field(update, i);
    ...
    ut_ad(n == n_fields + n_system_fields - n_skip_field);
    *upd = new_update;
  } else {
    *upd = update;
  }
...
/** Builds an update cts column vector based on a remaining part of an undo log record.
 @return remaining part of the record, NULL if an error detected, which
 means that the record is corrupted */
byte *trx_cts_update_rec_get_update(
    const byte *ptr,            /*!< in: remaining part in update undo log
                                record, after reading the row reference
                                NOTE that this copy of the undo log record must
                                be preserved as long as the update vector is
    ...
    upd_t **upd)               /*!< out, own: update vector */
{
  ...
  // Only update cts column
  update = upd_create(1, heap);


  /* Store cts to update vector */
  upd_field = upd_get_nth_field(update, 0);

  buf = static_cast<byte *>(mem_heap_alloc(heap, DATA_GTRX_ID_LEN));

  trx_write_cts(buf, trx->cts);
...
static MY_ATTRIBUTE((warn_unused_result)) bool trx_undo_get_undo_rec_ex(
    roll_ptr_t roll_ptr, bool is_history, trx_id_t trx_id, trx_id_t gtrx_id, mem_heap_t *heap, bool is_temp,
    const table_name_t &name, bool is_visible_user_table, trx_undo_rec_t **undo_rec, bool print_flag) {
  bool missing_history;

  rw_lock_s_lock(&purge_sys->latch);

  missing_history = purge_sys->view.changes_visible(trx_id, name) && purge_sys->view.changes_visible_cts(trx_id, is_history, gtrx_id, name, is_visible_user_table, nullptr);
  if (!missing_history) {
    *undo_rec = trx_undo_get_undo_rec_low(roll_ptr, heap, is_temp);
  } else {
	if (print_flag) {
		sql_print_warning("record(is history: %d) with cts %u is missing history.\n now oldest readview:\n", is_history, gtrx_id);
		purge_sys->view.print_sts();
	}
  }
...
bool trx_undo_prev_version_build_ex(
    ...
    lob::undo_vers_t *lob_undo, bool print_flag) {
  DBUG_TRACE;

  trx_undo_rec_t *undo_rec = nullptr;
  dtuple_t *entry;
  trx_id_t rec_trx_id;
  trx_id_t rec_gtrx_id = 0;
  ulint type;
  undo_no_t undo_no;
  table_id_t table_id;
  trx_id_t trx_id;
  ...
  *old_vers = nullptr;
  ...
  if (index->table->is_visible_user_table()) {
	  rec_gtrx_id = row_get_rec_gtrx_id(rec, index, offsets);
  }
  ...
  if (trx_undo_get_undo_rec_ex(roll_ptr, index_rec != rec, rec_trx_id, rec_gtrx_id, heap, is_temp,
                            index->table->name, index->table->is_visible_user_table(), &undo_rec, print_flag)) {
  ...
  ptr = trx_undo_rec_get_pars(undo_rec, &type, &cmpl_info, &dummy_extern,
                              &undo_no, &table_id, type_cmpl, &undo_rec_flags);
  ...
  ptr = trx_undo_update_rec_get_sys_cols(ptr, undo_rec_flags, &trx_id, &roll_ptr, &gtrx_id, &info_bits);
  ...
  ptr = trx_undo_update_rec_get_update(ptr, index, type, undo_rec_flags, gtrx_id, trx_id, roll_ptr,
                                       info_bits, nullptr, heap, &update, lob_undo,
                                       type_cmpl);
  ut_a(ptr);
  ...
  if (row_upd_changes_field_size_or_external(index, offsets, update)) {
    ...
    if ((update->info_bits & REC_INFO_DELETED_FLAG) &&
      ...
      missing_extern = purge_sys->view.changes_visible(trx_id, index->table->name) &&
          purge_sys->view.changes_visible_cts(trx_id, false, gtrx_id, index->table->name, index->table->is_visible_user_table(), nullptr);
...
bool trx_undo_prev_version_build(
    ...
    rec_t **old_vers, mem_heap_t *v_heap, const dtuple_t **vrow, ulint v_status,
    lob::undo_vers_t *lob_undo) {
	return trx_undo_prev_version_build_ex(index_rec,  index_mtr, rec,
		    index, offsets, heap, old_vers, v_heap, vrow, v_status, lob_undo, false);
}
