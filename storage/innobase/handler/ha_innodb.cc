...
/**This function is used to write the xid into the undo of an X/Open XA distributed transaction.
@return 0 or error number */
static
int
innobase_xa_ha_set_undo_xid_for_prepare(
/*================*/
...
/** This function is used to commit one X/Open XA distributed transaction
 which is in the prepared state
 @return 0 or error number */
static xa_status_code innobase_commit_by_xid_cts(
    handlerton *hton, /*!< in: InnoDB handlerton */
    XID *xid,         /*!< in: X/Open XA transaction
                      identification */
	uint64 cts);
...
/**This function is used to prepare one X/Open XA distributed transaction
which is in the prepared state during recover
@return 0 or error number */
static
xa_status_code
innobase_prepare_by_xid(
/*=====================*/
	handlerton*	hton,		/*!< in: InnoDB handlerton */
	XID*		xid);		/*!< in: X/Open XA transaction
					identification */
...
int convert_error_code_to_mysql(dberr_t error, uint32_t flags, THD *thd) {
  switch (error) {
    ...
    case DB_BLOCKING_READ_WAIT_TIMEOUT:
      return (HA_ERR_BLOCKING_READ_WAIT_TIMEOUT);
...
static int innodb_init(void *p) {
  ...
  innobase_hton->ha_set_undo_xid_for_prepare = innobase_xa_ha_set_undo_xid_for_prepare;
  innobase_hton->recover = innobase_xa_recover;
  innobase_hton->commit_by_xid = innobase_commit_by_xid;
  innobase_hton->commit_by_xid_cts = innobase_commit_by_xid_cts;
  innobase_hton->rollback_by_xid = innobase_rollback_by_xid;
  innobase_hton->prepare_by_xid = innobase_prepare_by_xid;
...
void add_cts_update_to_upd_vector(upd_t *update, byte *buf, trx_t *trx, const dict_index_t *index) {
  ...
  upd_field = upd_get_nth_field(update, update->n_fields - 1);
...
int ha_innobase::update_row(const uchar *old_row, uchar *new_row) {
  ...
  byte * cts_buf= nullptr;

  // add cts change into uvect here to add this change into the redo log
  if (uvect->table->is_visible_user_table()) {
	  cts_buf = (byte *)ut_malloc_nokey(DATA_GTRX_ID_LEN);
	  add_cts_update_to_upd_vector(uvect, cts_buf, trx, uvect->table->first_index());
  }
...
int innobase_basic_ddl::create_impl(THD *thd, const char *name, TABLE *form,
  ...
  if (dd_tab != nullptr && dd_tab->options().exists("visible_user_table")){
	info.set_user_visible_table_flags2();
  }
...
int ha_innobase::get_extra_columns_and_keys(const HA_CREATE_INFO *,
  ...
  if (dd_table->options().exists("visible_user_table")) {
	  /* Add the InnoDB system columns DB_GTRX_ID. */
	   dd::Column *db_gtrx_id = dd_add_hidden_column(
	       dd_table, "DB_GTRX_ID", DATA_GTRX_ID_LEN, dd::enum_column_types::LONGLONG);
	   if (db_gtrx_id == nullptr) {
	     return ER_WRONG_COLUMN_NAME;
	   }
	   dd_add_hidden_element(primary, db_gtrx_id);
  }
					...
					ended */
{
	...
	if (prepare_trx
		...
		dberr_t	err = trx_set_undo_xid_for_prepare_for_mysql(trx);
	...
	XID*		xid)	/*!< in: X/Open XA transaction identification */
{
  ...
  if (state == TRX_STATE_PREPARED)
    return(XA_OK);
  if (trx_prepare_for_mysql(trx) != DB_SUCCESS)
    return(XAER_NOTA);

  /*set is_xa_prepared_during_recover to be true, so that the ha_recover in
   * init_server_components, which has no binlog_xa_prepared will not rollback
   * this trx.*/
  trx->xid->set_is_xa_prepared_during_recover(true);
...
/** This function is used to commit one X/Open XA distributed transaction
 ...
 @return 0 or error number */
static xa_status_code innobase_commit_by_xid_cts(
    handlerton *hton, XID *xid, uint64 cts) /*!< in: X/Open XA transaction identification */
{
  assert(hton == innodb_hton_ptr);
...
static MYSQL_SYSVAR_ULONG(blocking_read_wait_timeout,
                          srv_blocking_read_wait_timeout,
                          PLUGIN_VAR_OPCMDARG,
                          "Timeout in microseconds an InnoDB transaction may wait "
                          "for another transaction commit or rollback. Max value is 24 hours.",
                          nullptr, nullptr, 100 * 1000, 1, 3600 * 24 * 1000, 0);

static MYSQL_SYSVAR_ULONG(purge_time_window,
                          srv_purge_time_window,
                          PLUGIN_VAR_OPCMDARG,
                          "Purge time window for computing oldest sts.",
                          nullptr, nullptr, 60, 1, 365 * 24 * 60 * 60, 0);

static MYSQL_SYSVAR_UINT(logical_time_size, srv_logical_time_size,
                         PLUGIN_VAR_OPCMDARG,
                         "Size of logical time in sts.", nullptr,
                         nullptr, 18, 0, 1024, 0);

static MYSQL_SYSVAR_BOOL(enable_tso, srv_enable_tso,
                         PLUGIN_VAR_OPCMDARG, "Switch for timestamp queries.",
                         nullptr, nullptr, TRUE);
...
static SYS_VAR *innobase_system_variables[] = {
    ...
    MYSQL_SYSVAR(blocking_read_wait_timeout),
    MYSQL_SYSVAR(purge_time_window),
    MYSQL_SYSVAR(logical_time_size),
    MYSQL_SYSVAR(enable_tso),
#ifdef UNIV_DEBUG
