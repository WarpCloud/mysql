...
#include "my_sm4.h"  // MY_SM4_IV_SIZE
...
/* Implementation of SM4 encryption routines */

/** helper class to process an IV argument to sm4_encrypt/sm4_decrypt */
class iv_argument_sm4 {
    char iv_buff[MY_SM4_IV_SIZE + 1];  // +1 to cater for the terminating NULL
    String tmp_iv_value;

public:
    iv_argument_sm4() : tmp_iv_value(iv_buff, sizeof(iv_buff), system_charset_info) {}
    ...
    /**
      ...
      @return a pointer to the retrived validated IV or NULL
    */
    const unsigned char *retrieve_iv_ptr(enum my_sm4_opmode sm4_opmode,
                                         uint arg_count, Item **args,
                                         const char *func_name, THD *thd,
                                         bool *error_generated) {
      const unsigned char *iv_str = nullptr;
...
String *Item_func_sm4_encrypt::val_str(String *str) {
  ...
  assert(sm4_opmode <= MY_SM4_END);

  if (sptr && key)  // we need both arguments to be not NULL
  {
    const unsigned char *iv_str =
        iv_arg.retrieve_iv_ptr((enum my_sm4_opmode)sm4_opmode, arg_count, args,
                               func_name(), thd, &null_value);
    if (null_value) return nullptr;

    // Calculate result length
    sm4_length =
        my_sm4_get_size(sptr->length(), (enum my_sm4_opmode)sm4_opmode);

    tmp_value.set_charset(&my_charset_bin);
    if (!tmp_value.alloc(sm4_length))  // Ensure that memory is free
    {
      // finally encrypt directly to allocated buffer.
      if (my_sm4_encrypt((unsigned char *)sptr->ptr(), sptr->length(),
                         (unsigned char *)tmp_value.ptr(),
                         (unsigned char *)key->ptr(), key->length(),
                         (enum my_sm4_opmode)sm4_opmode,
                         iv_str) == sm4_length) {
        // We got the expected result length
...
String *Item_func_sm4_decrypt::val_str(String *str) {
  ...
  if (sptr && key)  // Need to have both arguments not NULL
  {
    ...
    if (!str_value.alloc(sptr->length()))  // Ensure that memory is free
    {
      // finally decrypt directly to allocated buffer.
      int length;
      length = my_sm4_decrypt((unsigned char *)sptr->ptr(), sptr->length(),
                              (unsigned char *)str_value.ptr(),
                              (unsigned char *)key->ptr(), key->length(),
                              (enum my_sm4_opmode)sm4_opmode, iv_str);
      if (length >= 0)  // if we got correct data data
      {
        str_value.length((uint)length);
        return &str_value;
...
String *Item_func_translate::val_str(String *str) {
  ...
  const size_t from_length = res2->length();
  const char *search_end = search + from_length;
  const char *target = res3->ptr();
  const size_t to_length = res3->length();
  const char *target_end = target + to_length;
  const char *ptr = res1->ptr();
  const char *strend = res1->ptr() + res1->length();

  std::map<std::string, std::string> translate_meta;
  while (search < search_end) {
    String from, to;
    String *from_p = &from;
    String *to_p = &to;
    bool err = false;
    uint32 l2 = use_mb(res2->charset())
                ? my_ismbchar(res2->charset(), search, search_end)
                : 0;
    if (l2 != 0)
      while (l2-- > 0) err |= from_p->append(*search++);
    else
      err = from_p->append(*search++);
    if (err) return error_str();

    if (translate_meta.count(to_string(from))) {
      continue;
    }

    if (target < target_end) {
      uint32 l3 = use_mb(res3->charset())
                  ? my_ismbchar(res3->charset(), target, target_end)
                  : 0;
      if (l3 != 0)
        while (l3-- > 0) err |= to_p->append(*target++);
      else
        err = to_p->append(*target++);
      if (err) return error_str();
    }

    translate_meta.insert(std::make_pair(to_string(from), to_string(to)));
  }
  ...
  while (ptr < strend) {
    ...
    String *temp_p = &temp;
    if (l != 0)
      while (l-- > 0) err |= temp_p->append(*ptr++);
    else
      err = temp_p->append(*ptr++);
    if (err) return error_str();

    std::map<std::string, std::string>::iterator iter;
    iter = translate_meta.find(to_string(temp));
    if(iter != translate_meta.end()) {
      if (result->length() + iter->second.length() > max_size) {
        return push_packet_overflow_warning(thd, func_name());
      }
      if (result->append(iter->second.c_str())) return error_str();
    } else {
      err = result->append(temp);
      if (err) return error_str();
    }
  }
  return result;
