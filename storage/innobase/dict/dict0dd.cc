...
static inline dict_table_t *dd_fill_dict_table(const Table *dd_tab,
  ...
  unsigned flags2 = 0;
  if (dd_tab->table().options().exists("visible_user_table")) {
	  flags2 |= DICT_TF2_VISIBLE_USER_TABLE;
  }

  /* Create the dict_table_t */
  dict_table_t *m_table =
      dict_mem_table_create(norm_name, 0, n_cols, n_v_cols, n_m_v_cols, 0, flags2);
