...
struct xarecover_st {
  ...
  const std::map<XID, uint64> *binlog_xa_committed; //only used for start-up recover,
                                            // to record the commited xid from
                                            // binlog.
  const std::set<XID> *binlog_xa_rollbacked;
  const std::set<XID> *binlog_xa_prepared; //only used for start-up recover,
                                     // to record the prepared xid from binlog.
  bool dry_run;
};

bool operator<(const xid_t &xs1, const xid_t &xs2) {
  if (xs1.get_format_id() < xs2.get_format_id())
    return true;
  if (xs1.get_gtrid_length() < xs2.get_gtrid_length()) {
    return true;
  } else if (xs1.get_gtrid_length() > xs2.get_gtrid_length()) {
    return false;
  }
  if (xs1.get_bqual_length() < xs2.get_bqual_length()) {
    return true;
  } else if (xs1.get_bqual_length() > xs2.get_bqual_length()) {
    return false;
  }
	int ret = strncmp(xs1.get_data(), xs2.get_data(),
										xs1.get_gtrid_length() + xs1.get_bqual_length());
  if (ret < 0 )
    return true;
  return false;
}
...
//          info->found_foreign_xids++;
          ...
          if (info->binlog_xa_committed && !info->binlog_xa_committed->empty()) {
            std::map<XID, uint64>::const_iterator commit_it =
                info->binlog_xa_committed->find(target_xid);
            if (commit_it != info->binlog_xa_committed->end()) {
              uint64 cts = commit_it->second;
              if (cts > 0) {
                sql_print_information("commit xid %s with cts %llu due to find xa commit binlog with cts",
                		target_xid.xid_to_str(buf), cts);
                hton->commit_by_xid_cts(hton, &info->list[i].id, cts);
              } else {
            	sql_print_information("commit xid %s due to find xa commit binlog",
          ...
          if (info->binlog_xa_rollbacked && !info->binlog_xa_rollbacked->empty()) {
            std::set<XID>::const_iterator rollback_it =
            		info->binlog_xa_rollbacked->find(target_xid);
			if (rollback_it != info->binlog_xa_rollbacked->end()) {
			  sql_print_information("rollback xid %s due to find xa rollback binlog", target_xid.xid_to_str(buf));
			  hton->rollback_by_xid(hton, &info->list[i].id);
			  continue;
			}
          }
          if (info->binlog_xa_prepared && !info->binlog_xa_prepared->empty() && hton->prepare_by_xid) {
            std::set<XID>::const_iterator prepare_it =
                info->binlog_xa_prepared->find(target_xid);
            if (prepare_it != info->binlog_xa_prepared->end()) {
              if (info->list[i].id.get_is_xa_prepared_during_recover()) {
                sql_print_information("Find xid %s already prepare before current binlog", target_xid.xid_to_str(buf));
                if (Recovered_xa_transactions::instance().add_prepared_xa_transaction(
					  &info->list[i])) {
				  return true;
			    }
              ...
              sql_print_information("prepare xid %s due to find xa prepare binlog", target_xid.xid_to_str(buf));
              hton->prepare_by_xid(hton, &info->list[i].id);
              if (Recovered_xa_transactions::instance().add_prepared_xa_transaction(
				  &info->list[i])) {
			    return true;
			  }
          ...
          if (info->list[i].id.get_is_xa_prepared_during_recover()) {
            ...
            sql_print_information("rollback xid %s due to find xa transaction partial executed", target_xid.xid_to_str(buf));
            hton->rollback_by_xid(hton, &info->list[i].id);
          }
          continue;
        }
...
int ha_recover_ex(const mem_root_unordered_set<my_xid> *commit_list,
		const std::map<XID, uint64> *xa_committed, const std::set<XID> *xa_rollbacked, const std::set<XID> *xa_prepared) {
	xarecover_st info;
	DBUG_TRACE;
	...
	info.binlog_xa_rollbacked = xa_rollbacked;
...
int ha_recover(const mem_root_unordered_set<my_xid> *commit_list) {
  ...
  info.dry_run = (info.commit_list == nullptr &&
                  tc_heuristic_recover == TC_HEURISTIC_NOT_USED);
  info.list = nullptr;
  info.binlog_xa_committed = nullptr;
  info.binlog_xa_rollbacked = nullptr;
  info.binlog_xa_prepared = nullptr;
...
bool Sql_cmd_xa_commit::process_internal_xa_commit(THD *thd,
  ...
  if (xid_state->xa_trans_rolled_back()) {
    ...
    res = thd->is_error();
             ...
             m_xa_opt == XA_ONE_PHASE) {
	/*Before do commit one phase, we should first write the xid into undo log
	     * which is necessary for crash recover.*/
	int r= ha_set_undo_xid_for_prepare(thd);
	if ((res= r))
	    my_error(r == 1 ? ER_XA_RBROLLBACK : ER_XAER_RMERR, MYF(0));
	r= ha_commit_trans(thd, true);
...
bool Sql_cmd_xa_prepare::trans_xa_prepare(THD *thd) {
  ...
  if (!xid_state->has_state(XID_STATE::XA_IDLE))
    ...
    */
    ...
    if (thd->mdl_context.acquire_lock(&mdl_request,
                                      thd->variables.lock_wait_timeout) ||
    	/*before the redo and binlog flash in ha_prepare, store the xid into
    		         * undo*/
    	ha_set_undo_xid_for_prepare(thd) ||
        ha_xa_prepare(thd)) {
      /*
