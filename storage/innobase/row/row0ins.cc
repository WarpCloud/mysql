...
static void row_ins_alloc_sys_fields(ins_node_t *node) /*!< in: insert node */
{
  ...
  if (table->is_visible_user_table()){
	len += DATA_GTRX_ID_LEN;
  }
  ...
  if (table->is_visible_user_table()) {
	col = table->get_sys_col(DATA_GTRX_ID);

	dfield = dtuple_get_nth_field(row, dict_col_get_no(col));

	dfield_set_data(dfield, ptr, DATA_GTRX_ID_LEN);

	node->gtrx_id_buf = ptr;
  }
}
...
void add_cts_update_to_upd_vector_ex(upd_t *update, byte *buf, trx_t *trx, const dict_index_t *index) {
                               ...
                               latching any further pages */
{
  ...
  if (cursor->index->table->is_visible_user_table()) {
	  cts_buf = (byte *)ut_malloc_nokey(DATA_GTRX_ID_LEN);
	  add_cts_update_to_upd_vector_ex(update, cts_buf, thr_get_trx(thr), cursor->index);
  }
...
que_thr_t *row_ins_step(que_thr_t *thr) /*!< in: query thread */
{
  ...
  if (node->table->is_visible_user_table()) {
	  memset(node->gtrx_id_buf, 0, DATA_GTRX_ID_LEN);
	  trx_id_t new_cts;
	  if (node->table->skip_alter_undo) {
            /* non-online alter operation will skip undo log,
            so set gtrx_id in advance */
            new_cts = trx_sys_get_max_ts() + 1;
	  } else {
            new_cts = trx->cts;
	  }
	  trx_write_cts(node->gtrx_id_buf, new_cts);
  }
