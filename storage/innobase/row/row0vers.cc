...
/** Finds out if we must preserve a delete marked earlier version of a clustered
 ...
 @param[in]	gtrx_id		global transaction id in the version
...
ibool row_vers_must_preserve_del_marked(trx_id_t trx_id, trx_id_t gtrx_id,
                                        const table_name_t &name, bool is_user_visible_table, mtr_t *mtr) {
  ut_ad(!rw_lock_own(&(purge_sys->latch), RW_LOCK_S));

  mtr_s_lock(&purge_sys->latch, mtr);

  return (!purge_sys->view.changes_visible(trx_id, name) && !purge_sys->view.changes_visible_cts(trx_id, true, gtrx_id, name, is_user_visible_table, nullptr));
}
...
dberr_t row_vers_build_for_consistent_read(
  ...
  ut_ad(!view->changes_visible_cts(trx_id, false, gtrx_id, index->table->name, index->table->is_visible_user_table(), nullptr));
  ...
  for (;;) {
    ...
    bool purge_sees =
        trx_undo_prev_version_build_ex(rec, mtr, version, index, *offsets, heap,
                                    &prev_version, nullptr, vrow, 0, lob_undo, true);

    if (!purge_sees) {
    	sql_print_error("DB_MISSING_HISTORY");
    	sql_print_warning("current transaction read view:\n");
    	view->print_sts();
    }

    //TODO temp ignore DB_MISSING_HISTORY, afterward should use another error code
//    err = (purge_sees) ? DB_SUCCESS : DB_MISSING_HISTORY;
    ...
    *offsets = rec_get_offsets(prev_version, index, *offsets, ULINT_UNDEFINED,
    ...
    if (index->table->is_visible_user_table()) {
	  gtrx_id = row_get_rec_gtrx_id(prev_version, index, *offsets);
    }

    if (view->changes_visible_cts(trx_id, true, gtrx_id, index->table->name, index->table->is_visible_user_table(), nullptr)) {
