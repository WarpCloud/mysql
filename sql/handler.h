...
typedef int  (*ha_set_undo_xid_for_prepare_t)(handlerton *hton, THD *thd, bool all);
...
typedef xa_status_code (*commit_by_xid_cts_t)(handlerton *hton, XID *xid, uint64 cts);

typedef xa_status_code (*rollback_by_xid_t)(handlerton *hton, XID *xid);

typedef xa_status_code  (*prepare_by_xid_t)(handlerton *hton, XID *xid);
...
struct handlerton {
  ...
  */
  ...
  rollback_t rollback;
  prepare_t prepare;
  ha_set_undo_xid_for_prepare_t ha_set_undo_xid_for_prepare;
  recover_t recover;
  commit_by_xid_t commit_by_xid;
  commit_by_xid_cts_t commit_by_xid_cts;
  rollback_by_xid_t rollback_by_xid;
  prepare_by_xid_t prepare_by_xid;
  create_t create;
...
int ha_set_undo_xid_for_prepare(THD *thd);
...
int ha_recover_ex(const mem_root_unordered_set<my_xid> *commit_list,
               const std::map<XID, uint64> *xa_committed = NULL, const std::set<XID> *xa_rollbacked = NULL, const std::set<XID> *xa_prepared = NULL);
