...
#define SETMSG(nr, msg) handler_errmsgs[(nr)-HA_ERR_FIRST] = (msg)
  ...
  SETMSG(HA_ERR_BLOCKING_READ_WAIT_TIMEOUT, ER_DEFAULT(ER_BLOCKING_READ_WAIT_TIMEOUT));
...
/**
*  Only store the xid into undo log, this func is used before doing real
*  prepare or commit one phase, to store the xid in undo before flush redo
*  log in ordered_commit.
 @retval
   0   ok
 @retval
   1   error, transaction was rolled back
...
int ha_set_undo_xid_for_prepare(THD *thd)
{
 int error=0;
 Transaction_ctx *trn_ctx= thd->get_transaction();
 DBUG_ENTER("ha_set_undo_xid_for_prepare");
 ...
 if (trn_ctx->is_active(Transaction_ctx::SESSION))
 {
   ...
   while (ha_info)
   {
     ...
     if (ht->ha_set_undo_xid_for_prepare)
     {
       if (ht->ha_set_undo_xid_for_prepare(ht, thd, true))
       {
         ha_rollback_trans(thd, true);
         error=1;
         break;
...
void handler::print_error(int error, myf errflag) {
  ...
  switch (error) {
    ...
    case HA_ERR_TOO_LONG_PATH:
      ...
      break;
    case HA_ERR_BLOCKING_READ_WAIT_TIMEOUT:
      textno = ER_BLOCKING_READ_WAIT_TIMEOUT;
      break;
    default: {
