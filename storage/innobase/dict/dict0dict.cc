...
void dict_table_add_system_columns(dict_table_t *table, mem_heap_t *heap) {
  ...
  if (table->is_visible_user_table()) {
	  dict_mem_table_add_col(table, heap, "DB_GTRX_ID", DATA_SYS,
	      					   DATA_GTRX_ID | DATA_NOT_NULL, DATA_GTRX_ID_LEN, false);
  }
                               ...
                               a clustered index */
{
  ...
  if (table->is_visible_user_table()) {
	dict_index_add_col(new_index, table, table->get_sys_col(DATA_GTRX_ID), 0,
						 true);
  }
