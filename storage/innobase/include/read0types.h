...
class ReadView {
 ...
 public:
  ...
  void print_sts() {
	sql_print_warning("sts: %llu", m_sts);
  }
  ...
  /** Check whether the changes by id and cts are visible.
   ...
   @param[in]	is_history	 whether the row is history version
   @param[in]	cts	     commit timestamp to check against the view
   @param[in]	name	table name
   @param[in,out]	*error_code  0: success, 1: timeout, 2: cts_cache.Get(id) failed
   @return whether the view sees the modifications of id. */
  bool changes_visible_cts(trx_id_t id,
		  bool is_history,
			trx_id_t cts,
  ...
  @param id		transaction to check
  @return true if view sees transaction id */
  bool sees_global(trx_id_t id) const {
	if (m_sts != 0) {
		// there is not max gxid stored in second index page header, so can not check whether to see the record
	    // so return false to search the clustered record instead.
	    return false;
	}
