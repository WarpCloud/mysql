  ...
  if (!m_trx) {
    ...
    if (m_config.m_index->is_clustered()) {
      ...
      if (m_config.m_index->trx_id_offset > 0) {
        ...
        if (m_config.m_index->table->is_visible_user_table()) {
        	rec_gtrx_id = trx_read_cts(rec + m_config.m_index->trx_id_offset + DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN);
        }
      } else {
        rec_trx_id = row_get_rec_trx_id(rec, m_config.m_index, offsets);
        if (m_config.m_index->table->is_visible_user_table()) {
			rec_gtrx_id = row_get_rec_gtrx_id(rec, m_config.m_index, offsets);
		}
      }

      if (m_trx->isolation_level > TRX_ISO_READ_UNCOMMITTED &&
          !view->changes_visible_cts(rec_trx_id, false, rec_gtrx_id, table_name,m_config.m_index->table->is_visible_user_table(), nullptr)) {
        rec_t *old_vers;
