...
/** Checks that a record is seen in a consistent read.
 @return 1 if sees,
         0 if an earlier version of the record should be retrieved
         >1 if this trx id should be waited for its commit or rollback */
trx_id_t lock_clust_rec_cons_read_sees_ex(
    ...
    ReadView *view)       /*!< in: consistent read view */
{
  ...
  if (index->table->is_visible_user_table()) {
	  gtrx_id = row_get_rec_gtrx_id(rec, index, offsets);
  }

  int error_code = 0;
  bool sees = false;

  sees = view->changes_visible_cts(trx_id, false, gtrx_id, index->table->name, index->table->is_visible_user_table(), &error_code);
    ...
    ReadView *view)       /*!< in: consistent read view */
{
  ...
  return (view->changes_visible_cts(trx_id, false, gtrx_id, index->table->name, index->table->is_visible_user_table(), nullptr));
}
    ...
    const ReadView *view)      /*!< in: consistent read view */
{
  ...
  return (view->sees_global(max_trx_id));
}
...
void lock_print_info_summary(FILE *file) {
  ...
  fputs(
    "---------------------\n"
    "BLOCKING READ RELATED\n"
    "---------------------\n",
    file);

  fprintf(file, "Number of current blocking read trx  " TRX_ID_FMT "\n",
          uint64(blocking_read_trx_current.load()));

  fprintf(file, "Number of times of total blocking read timeout  " TRX_ID_FMT "\n",
          uint64(blocking_read_timeout_total.load()));

  fprintf(file, "Number of times of total blocking read success and timeout trx  " TRX_ID_FMT "\n",
          uint64(blocking_read_trx_total.load()));

  fprintf(file, "Number of times of total blocking read trx without timeout  " TRX_ID_FMT "\n",
          uint64(blocking_read_trx_total_without_timeout.load()));

  fprintf(file, "Number of total blocking read time  " TRX_ID_FMT " ns\n",
          uint64(blocking_read_time_total.load()));

  fprintf(file, "Number of total blocking read time without timeout part  " TRX_ID_FMT " ns\n",
          uint64(blocking_read_time_total_without_timeout.load()));

  fprintf(file, "Global oldest sts is  " TRX_ID_FMT "\n", uint64(global_m_sts.load()));

  fprintf(file, "The max ts  " TRX_ID_FMT "\n", trx_sys_get_max_ts());
  ...
  fputs(
    "--------------------------------------------\n"
    "Trx status cache using atomic, without mutex\n"
    "--------------------------------------------\n",
    file);

  fputs(
  "** TRX_SYS_STATUS_CACHE_SIZE is the capacity of trx_sys->status_cache **\n",
  file);

  uint64 running_in_trx_sys_status_cache_current = uint64(running_in_trx_sys_status_cache.load());
  uint64 prepared_in_trx_sys_status_cache_current = uint64(prepared_in_trx_sys_status_cache.load());
  uint64 rollback_in_trx_sys_status_cache_current = uint64(rollback_in_trx_sys_status_cache.load());
  uint64 cts_in_trx_sys_status_cache_current = uint64(cts_in_trx_sys_status_cache.load());

  fprintf(file, "The value of macro TRX_SYS_STATUS_CACHE_SIZE is  " TRX_ID_FMT "\n",
          uint64(TRX_SYS_STATUS_CACHE_SIZE));

  fprintf(file, "Number of trx RUNNING in trx_sys->status_cache  " TRX_ID_FMT "\n",
          running_in_trx_sys_status_cache_current);

  fprintf(file, "Number of trx PREPARED in trx_sys->status_cache  " TRX_ID_FMT "\n",
          prepared_in_trx_sys_status_cache_current);

  fprintf(file, "Number of trx ROLLBACK in trx_sys->status_cache  " TRX_ID_FMT "\n",
          rollback_in_trx_sys_status_cache_current);

  fprintf(file, "Number of trx CTS in the trx_sys->status_cache  " TRX_ID_FMT "\n",
          cts_in_trx_sys_status_cache_current);


  fputs(
    "--------------------------------------------\n"
    "----------------------------\n"
    "Trx status cache using mutex\n"
    "----------------------------\n",
    file);

  trx_sys_status_mutex_enter();

  uint64 running_in_evicted_status_map_current = uint64(running_in_evicted_status_map.load());
  uint64 prepared_in_evicted_status_map_current = uint64(prepared_in_evicted_status_map.load());
  uint64 rollback_in_evicted_status_map_current = uint64(rollback_in_evicted_status_map.load());
  uint64 cts_in_evicted_status_map_current = uint64(cts_in_evicted_status_map.load());

  fprintf(file, "Size of trx_sys->evicted_status_map  " TRX_ID_FMT "\n",
          trx_sys->evicted_status_map.size());

  fprintf(file, "Historic high size of trx_sys->evicted_status_map  " TRX_ID_FMT "\n",
          max_size_evicted_status_map);

  fprintf(file, "Number of trx RUNNING in trx_sys->evicted_status_map  " TRX_ID_FMT "\n",
          running_in_evicted_status_map_current);

  fprintf(file, "Number of trx PREPARED in trx_sys->evicted_status_map  " TRX_ID_FMT "\n",
          prepared_in_evicted_status_map_current);

  fprintf(file, "Number of trx ROLLBACK in trx_sys->evicted_status_map  " TRX_ID_FMT "\n",
          rollback_in_evicted_status_map_current);

  fprintf(file, "Number of trx CTS in trx_sys->evicted_status_map  " TRX_ID_FMT "\n",
          cts_in_evicted_status_map_current);
  ...
  fputs(
    ...
    "-----------------------\n"
    "** Status in trx_sys->status_cache and trx_sys->evicted_status_map **\n",
    file);

  fprintf(file, "Number of total trx RUNNING  " TRX_ID_FMT "\n",
          uint64(running_in_evicted_status_map_current + running_in_trx_sys_status_cache_current));

  fprintf(file, "Number of total trx PREPARED  " TRX_ID_FMT "\n",
          uint64(prepared_in_evicted_status_map_current + prepared_in_trx_sys_status_cache_current));

  fprintf(file, "Number of total trx ROLLBACK  " TRX_ID_FMT "\n",
          uint64(rollback_in_evicted_status_map_current + rollback_in_trx_sys_status_cache_current));

  fprintf(file, "Number of total trx CTS  " TRX_ID_FMT "\n",
          uint64(cts_in_evicted_status_map_current + cts_in_trx_sys_status_cache_current));
