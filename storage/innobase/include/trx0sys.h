...
/** Waiting that the prepared transaction transformed.
 ...
 @param[in]   thr  current thread
 @return status or cts that the prepared transaction has transformed into*/
...
/** Clean the transaction status or cts from cache.
 @param[in]   id   trx id
 */
void trx_sys_clean_trx_status(const trx_id_t id);

/** Get the transaction status or cts and sts from cache.
 @param[in]   id   trx id
 @param[out]   status_or_cts   0: not found in the cache; 1: RUNNING; 2: PREPARED; other: CTS
 @param[out]   sts
 @return true if id is found */
bool trx_sys_get_trx_status(const trx_id_t id, trx_id_t *status_or_cts, trx_id_t *sts);

/** The following four functions increase or decrease
    the corresponding variable according to the value of the parameter:
    {running | prepared | rollback | cts}_in_trx_sys_status_cache,
    {running | prepared | rollback | cts}_in_evicted_status_map.
 @param[in] status_or_cts   0: not found in the cache; 1: RUNNING; 2: PREPARED; other: CTS */
...
/** Update the rw transaction status or cts from cache.
 @param[in]   trx   trx_t
 @param[in]   status_or_cts  transaction status or cts. 0: not found in the cache; 1: RUNNING; 2: PREPARED; other: CTS
 */
void trx_sys_update_trx_status(const trx_t *trx, const trx_id_t status_or_cts);

/** Update the rw transaction status or cts and sts from cache.
 @param[in]   trx   trx_t
 @param[in]   status_or_cts  transaction status or cts. 0: not found in the cache; 1: RUNNING; 2: PREPARED; other: CTS
 @param[in]   sts            transaction sts, used for share transaction
 */
void trx_sys_update_trx_status(const trx_t *trx, const trx_id_t status_or_cts, const trx_id_t sts);

/** Get the max timestamp got from TSO.
 @return max_ts */
UNIV_INLINE
trx_id_t trx_sys_get_max_ts();


/** Update the max timestamp got from TSO.
@param[in]	ts	timestamp from TSO; sts or cts */
UNIV_INLINE
void trx_sys_update_max_ts(trx_id_t ts);
...
#define TRX_SYS_STATUS_CACHE_SIZE (10000)

#define TRX_SYS_STATUS_NONE ((trx_id_t)0)

#define TRX_SYS_STATUS_RUNNING ((trx_id_t)1)

#define TRX_SYS_STATUS_PREPARED ((trx_id_t)2)

#define TRX_SYS_STATUS_ROLLBACK ((trx_id_t)3)

#define TRX_SYS_MAX_TS_INIT ((trx_id_t)10)

struct trx_status_t {
	trx_status_t(trx_id_t id, trx_id_t status_or_cts) {
		this->id = id;
		this->status_or_cts = status_or_cts;
		this->sts = 0;
	}

	trx_status_t(trx_id_t id, trx_id_t status_or_cts, trx_id_t sts) {
		this->id = id;
		this->status_or_cts = status_or_cts;
		this->sts = sts;
	}
	...
	trx_id_t id;
	// 0: not found in the cache; 1: RUNNING; 2: PREPARED; other: CTS
	trx_id_t status_or_cts;

	trx_id_t sts;
};

struct trx_status_sts_t {
	trx_status_sts_t(trx_id_t status_or_cts, trx_id_t sts) {
		this->status_or_cts = status_or_cts;
		this->sts = sts;
	}
	trx_id_t status_or_cts;
...
extern std::atomic<int> running_in_trx_sys_status_cache;
extern std::atomic<int> prepared_in_trx_sys_status_cache;
extern std::atomic<int> rollback_in_trx_sys_status_cache;
extern std::atomic<int> cts_in_trx_sys_status_cache;

// Statistics for four kind of types in trx_sys->evicted_status_map
extern std::atomic<int> running_in_evicted_status_map;
extern std::atomic<int> prepared_in_evicted_status_map;
extern std::atomic<int> rollback_in_evicted_status_map;
extern std::atomic<int> cts_in_evicted_status_map;

// Recored the historical maximum value of size of trx_sys->evicted_status_map
extern uint64 max_size_evicted_status_map;

// Statistics for blocking read related
// Recored number of current blocking read trx
extern std::atomic<unsigned long long> blocking_read_trx_current;
// Recored number of times of total blocking read success and timeout trx
extern std::atomic<unsigned long long> blocking_read_trx_total;
// Recored number of times of total blocking read trx without timeout
extern std::atomic<unsigned long long> blocking_read_trx_total_without_timeout;
// Recored number of total blocking read time
extern std::atomic<unsigned long long> blocking_read_time_total;
// Recored number of total blocking read time without timeout part
extern std::atomic<unsigned long long> blocking_read_time_total_without_timeout;
// Recored number of times of total blocking read timeout
extern std::atomic<unsigned long long> blocking_read_timeout_total;
...
struct trx_sys_t {
  ...
  trx_id_t max_ts;         /*!< Max timestamp got from TSO. */

  std::atomic<trx_status_t> status_cache[TRX_SYS_STATUS_CACHE_SIZE];

  std::unordered_map<trx_id_t, trx_status_sts_t*> evicted_status_map;

  TrxSysMutex trx_status_mutex;

  // used to store threads which are waiting for transaction's
  // query state == commit or rollback
  std::map<trx_id_t, std::vector<que_thr_t*>*> trx_wait_event_map;

  // mutex protects above variables
  TrxWaitEventMutex trx_wait_event_mutex;
};
...
/** Acquire the trx_sys->mutex. */
#define trx_sys_status_mutex_enter()     \
  do {                            \
    mutex_enter(&trx_sys->trx_status_mutex); \
  } while (0)

/** Release the trx_sys->mutex. */
#define trx_sys_status_mutex_exit() \
  do {                       \
    trx_sys->trx_status_mutex.exit();   \
  } while (0)
