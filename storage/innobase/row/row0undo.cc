...
cts_node_t *row_cts_node_create(trx_t *trx, mem_heap_t *heap) {
	cts_node_t *cts_node;

   ut_ad(trx_state_eq(trx, TRX_STATE_ACTIVE) ||
         trx_state_eq(trx, TRX_STATE_PREPARED));

   cts_node = static_cast<cts_node_t *>(mem_heap_alloc(heap, sizeof(cts_node_t)));

   cts_node->common.type = QUE_NODE_CTS;

   cts_node->state = UNDO_NODE_FETCH_NEXT;
   cts_node->trx = trx;

   btr_pcur_init(&(cts_node->pcur));

   cts_node->heap = mem_heap_create(256);
...
/** Looks for the clustered index record when node has the row reference.
 ...
 @return true if found; NOTE the node->pcur must be closed by the
 caller, regardless of the return value */
bool row_cts_search_clust_to_pcur(
    cts_node_t *node) /*!< in/out: row cts node */
{
  dict_index_t *clust_index;
  bool found;
  mtr_t mtr;
  ...
  if (found) {
    ...
    // not to update info bits
    if (node->update != nullptr) {
    	node->update->info_bits = node->row->info_bits;
    }
...
/** Fetches an undo log record and find the referenced row to update its cts.
 If none left, or a partial rollback completed, returns control to the
 parent node, which is always a query thread node.
 @return DB_SUCCESS if operation successfully completed, else error code */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_cts(cts_node_t *node, /*!< in: row cts node */
             que_thr_t *thr)    /*!< in: query thread */
{
  dberr_t err;
  trx_t *trx;
  ...
  if (node->state == UNDO_NODE_FETCH_NEXT) {
    ...
    if (!node->undo_rec) {
      /* Setting cts completed for this query thread */
  ...
  /* During setting cts, trx is holding at least LOCK_IX on each
  modified table. It may also hold MDL. A concurrent DROP TABLE
  or ALTER TABLE should be impossible, because it should be
  holding both LOCK_X and MDL_EXCLUSIVE on the table. */
  if (node->state == UNDO_NODE_INSERT) {
    err = row_cts_ins(node, thr);

    node->state = UNDO_NODE_FETCH_NEXT;
  } else {
    ut_ad(node->state == UNDO_NODE_MODIFY);
    err = row_cts_mod(node, thr);
  }
...
/** Set cts into a row operation in a table. This is a high-level function used
 in SQL execution graphs.
 @return query thread to run next or NULL */
que_thr_t *row_cts_step(que_thr_t *thr) /*!< in: query thread */
{
  dberr_t err;
  cts_node_t *node;
  trx_t *trx;
  ...
  node = static_cast<cts_node_t *>(thr->run_node);

  ut_ad(que_node_get_type(node) == QUE_NODE_CTS);

  err = row_cts(node, thr);
  ...
  if (err != DB_SUCCESS) {
    ...
    if (err == DB_OUT_OF_FILE_SPACE) {
      ib::fatal(ER_IB_MSG_1041) << "Out of tablespace during setting cts."
                                   " Consider increasing your tablespace.";
    }

    ib::fatal(ER_IB_MSG_1042)
        << "Error (" << ut_strerr(err) << ") in setting cts.";
  }
