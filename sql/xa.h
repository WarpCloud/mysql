...
typedef struct xid_t {
  ...
  */
  ...
  /**
   *  Flag indicate this xa transaction is a prepared transaction before
   *  server start-up. is_xa_prepared_during_recover is only used for recover
   *  process during server start-up.*/
  bool is_xa_prepared_during_recover;

 public:
  xid_t() : formatID(-1), gtrid_length(0), bqual_length(0) {
    memset(data, 0, XIDDATASIZE);
    is_xa_prepared_during_recover =  false;
  }

  void set_is_xa_prepared_during_recover(bool b)
  {
    is_xa_prepared_during_recover = b;
  }

  bool get_is_xa_prepared_during_recover()
  {
    return is_xa_prepared_during_recover;
  }
...
extern bool operator<(const xid_t &xs1, const xid_t &xs2);
extern bool operator==(const xid_t &xs1, const xid_t &xs2);
