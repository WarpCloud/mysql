...
class Row_sel_get_clust_rec_for_mysql {
 ...
 public:
  ...
  dberr_t operator()(row_prebuilt_t *prebuilt, dict_index_t *sec_index,
                     ...
                     lob::undo_vers_t *lob_undo, trx_id_t *wait_trx_id);
};
  ...
  *offsets = rec_get_offsets(clust_rec, clust_index, *offsets, ULINT_UNDEFINED,
  ...
  if (prebuilt->select_lock_type != LOCK_NONE) {
    ...
    if (trx->isolation_level > TRX_ISO_READ_UNCOMMITTED) {
      ...
      if (wait_trx_id == nullptr) {
    	  if (!lock_clust_rec_cons_read_sees(clust_rec, clust_index, *offsets,
    	  ...
    	  trx_id_t sees_or_wait = lock_clust_rec_cons_read_sees_ex(clust_rec, clust_index, *offsets,
							trx_get_read_view(trx));
	...
	trx_id_t *wait_trx_id)
{
  ...
  *offsets = rec_get_offsets(rec, index, *offsets, ULINT_UNDEFINED, heap);
  ...
  trx_id_t sees_or_waits = lock_clust_rec_cons_read_sees_ex(rec, index, *offsets,
          trx_get_read_view(trx));

  if (sees_or_waits == 0) {
	  return (SEL_RETRY);
  } else if (sees_or_waits > 1) {
	  *wait_trx_id = sees_or_waits;
	  return (SEL_RETRY);
  }
//  if (!lock_clust_rec_cons_read_sees(rec, index, *offsets,
//                                     trx_get_read_view(trx))) {
        ...
        case SEL_RETRY:
          if (wait_trx_id > 1) {
			  ...
			  trx->has_search_latch = false;
                          ...
                          if (trx_sys_wait_trx_commit(wait_trx_id, thr) != DB_BLOCKING_READ_WAIT_TIMEOUT) {
                            goto retry_search_shortcut;
                          }
                          err = DB_BLOCKING_READ_WAIT_TIMEOUT;
                          trx->error_state = err;
                          que_thr_stop_for_mysql(thr);
                          goto func_exit;
//                          trx_sys_wait_trx_commit(wait_trx_id);
//                          goto retry_search_shortcut;
          }
          break;
  ...
  locks_ok:
    ...
    if (trx->isolation_level == TRX_ISO_READ_UNCOMMITTED) {
      ...
      if (srv_force_recovery < 5) {
    	...
    	trx_id_t sees_or_wait = lock_clust_rec_cons_read_sees_ex(rec, index, offsets,
                trx_get_read_view(trx));
    	if (sees_or_wait == 0) {
    		rec_t *old_vers;
			/* The following call returns 'offsets' associated with 'old_vers' */
