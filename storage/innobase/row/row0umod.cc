...
/** Setting cts into a modify in a clustered index record.
 @return DB_SUCCESS, DB_FAIL, or error code: we may run out of file space */
static MY_ATTRIBUTE((warn_unused_result)) dberr_t row_cts_mod_clust_low(
    cts_node_t *node, /*!< in: row cts node */
    ulint **offsets,   /*!< out: rec_get_offsets() on the record */
    mem_heap_t **offsets_heap,
    /*!< in/out: memory heap that can be emptied */
    mem_heap_t *heap, /*!< in/out: memory heap */
    ...
    ulint mode)        /*!< in: BTR_MODIFY_LEAF or BTR_MODIFY_TREE */
{
  ...
  if (!btr_pcur_restore_position(mode, &node->pcur, mtr) ||
      row_vers_must_preserve_del_marked(node->new_trx_id, node->new_gtrx_id, node->table->name,
    		  node->table->is_visible_user_table(), mtr)) {
    return (DB_SUCCESS);
  }
                       ...
                       que_thr_t *thr)    /*!< in: query thread */
{
  ...
  ut_ad(rec_get_gtrx_id(btr_pcur_get_rec(pcur), index) == node->new_gtrx_id);
...
/** Setting cts into a modify in a clustered index record. Sets also the node state for
 the next round of undo.
 @return DB_SUCCESS or error code: we may run out of file space */
MY_ATTRIBUTE((warn_unused_result)) dberr_t
    row_cts_mod_clust(cts_node_t *node, /*!< in: row cts node */
                       que_thr_t *thr)    /*!< in: query thread */
{
  btr_pcur_t *pcur;
  mtr_t mtr;
  ...
  if (err != DB_SUCCESS) {
    ...
    //TODO TSO
    err = row_cts_mod_clust_low(node, &offsets, &offsets_heap, heap,
                                 &rebuilt_old_pk, sys, thr, &mtr,
                                 BTR_MODIFY_TREE);
    ut_ad(err == DB_SUCCESS || err == DB_OUT_OF_FILE_SPACE);
  }
...
static void row_undo_mod_parse_undo_rec(undo_node_t *node, THD *thd,
  ...
  ptr = trx_undo_update_rec_get_update(ptr, clust_index, type, undo_rec_flags, gtrx_id, trx_id, roll_ptr,
                                       info_bits, node->trx, node->heap,
                                       &(node->update), nullptr, type_cmpl);

  node->new_trx_id = trx_id;
  node->new_gtrx_id = gtrx_id;
...
/** Parses the row reference and other info in a modify undo log record.
@param[in]	node	row cts node
@param[in]      thd     THD associated with the node
@param[in,out]	mdl	MDL ticket or nullptr if unnecessary */
static void row_cts_mod_parse_undo_rec(cts_node_t *node, THD *thd,
                                        MDL_ticket **mdl) {
  dict_index_t *clust_index;
  byte *ptr;
  undo_no_t undo_no;
  ...
  if (node->table->ibd_file_missing || !node->table->is_visible_user_table()) {
    dd_table_close(node->table, thd, mdl, false);
  ...
  ptr = trx_cts_update_rec_get_update(ptr, clust_index, node->trx, node->heap,
                                       &(node->update));

  if (!row_cts_search_clust_to_pcur(node)) {
    dd_table_close(node->table, thd, mdl, false);
...
/** Setting cts into a modify operation on a row of a table.
 @return DB_SUCCESS or error code */
dberr_t row_cts_mod(cts_node_t *node, /*!< in: row cts node */
                     que_thr_t *thr)    /*!< in: query thread */
{
  dberr_t err;
  MDL_ticket *mdl = nullptr;
  ...
  row_cts_mod_parse_undo_rec(node, thd,
                              dd_mdl_for_undo(node->trx) ? &mdl : nullptr);
  ...
  node->index = node->table->first_index();
  ut_ad(node->index->is_clustered());
  err = row_cts_mod_clust(node, thr);
