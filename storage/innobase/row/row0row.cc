...
static inline dtuple_t *row_build_low(ulint type, const dict_index_t *index,
  ...
  if (!index->table->is_visible_user_table() && col_table->is_visible_user_table()) {
	  ...
	  dict_col_t *col = col_table->get_sys_col(DATA_GTRX_ID);
	  dfield_t *dfield = dtuple_get_nth_field(row, dict_col_get_no(col));
	  dfield_set_data(dfield, buf, DATA_GTRX_ID_LEN);
  }
