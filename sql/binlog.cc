...
// the format in binlog is: XA COMMIT X'31',X'32',1 cts 10000
static XID get_xid_from_xa_commit_rollback(std::string query_sql, uint64 *cts) {
  XID xid;
  size_t gtrid_begin_pos = query_sql.find('\'', 0);
  size_t gtrid_end_pos = query_sql.find('\'', gtrid_begin_pos + 1);
  std::string gtrid;
  for (size_t i = 0; i < gtrid_end_pos - gtrid_begin_pos - 1; i += 2) {
    gtrid += (char)(char_val(query_sql[gtrid_begin_pos + 1 + i]) * 16 +
                    char_val(query_sql[gtrid_begin_pos + 1 + i + 1]));
  }
  size_t bqual_begin_pos = query_sql.find('\'', gtrid_end_pos + 1);
  size_t bqual_end_pos = query_sql.find('\'', bqual_begin_pos + 1);
  std::string bqual;
  for (size_t i = 0; i < bqual_end_pos - bqual_begin_pos - 1; i += 2) {
    bqual += (char)(char_val(query_sql[bqual_begin_pos + 1 + i]) * 16 +
                    char_val(query_sql[bqual_begin_pos + 1 + i + 1]));
  }
  size_t format_id_pos = query_sql.find(',', bqual_end_pos + 1);
  long format_id = atol(query_sql.c_str() + format_id_pos + 1);

  if (cts != nullptr) {
	  std::string::size_type cts_pos = query_sql.find("CTS");
	  if (cts_pos != query_sql.npos) {
		  *cts = std::strtoull(query_sql.c_str() + cts_pos + 3 + 1, nullptr, 10);
	  }
  }

  xid.set(format_id, gtrid.c_str(), gtrid.size(), bqual.c_str(), bqual.size());
  return xid;
}
...
class binlog_cache_data {
  ...
  */
  virtual void reset() {
    ...
    flags.with_xa_prepare= false;
    flags.with_xa_commit= false;
    flags.with_xa_rollback= false;
  ...
  bool has_empty_transaction() {
    ...
    */
        ...
        !flags.with_content)  // Has no other content than START/END
    {
      ...
      assert(!flags.with_xa_prepare);
      assert(!flags.with_xa_commit);
      assert(!flags.with_xa_rollback);
      return true;
    }
    return false;
  }
  ...
  */
  ...
  struct Flags {
    ...
    */
    ...
    /*
      This indicates that the cache contain an XA PREPARE event.
    */
    bool with_xa_prepare:1;

    /*
      This indicates that the cache contain an XA COMMIT event.
    */
    bool with_xa_commit:1;

    /*
      This indicates that the cache contain an XA ROLLBACK event AFTER PREPARE.
    */
    bool with_xa_rollback:1;
  ...
  if (ev != nullptr) {
    ...
    if (ev->get_type_code() == binary_log::XID_EVENT)
      ...
      flags.with_xa_prepare= true;
        ...
        !native_strncasecmp(((Query_log_event *)ev)->query,
                            STRING_WITH_LEN("XA COMMIT")))
      flags.with_xa_commit= true;
    else if (ev->get_type_code() == binary_log::QUERY_EVENT &&
        !native_strncasecmp(((Query_log_event *)ev)->query,
                            STRING_WITH_LEN("XA ROLLBACK")))
      flags.with_xa_rollback= true;
  ...
  */
  ...
  if (flags.finalized) {
    ...
    */
    ...
    if ((flags.with_xid || flags.with_xa_prepare || flags.with_xa_commit || flags.with_xa_rollback) && error == 0) *wrote_xid = true;
...
inline int do_binlog_xa_commit_rollback(THD *thd, XID *xid, bool commit) {
  ...
  if (commit && thd->tx_cts > 0) {
	std::string str = std::to_string(thd->tx_cts);
	char query[(sizeof("XA COMMIT")) + 1 + sizeof(buf) + 1 + sizeof(str.c_str())];
	int qlen = sprintf(query, "XA COMMIT %s CTS %s", xid->serialize(buf), str.c_str());
...
static bool binlog_recover(Binlog_file_reader *binlog_file_reader,
  ...
  */
  ...
  int memory_page_size = my_getpagesize();
  {
    ...
    std::set<XID> xa_prepared;
    std::map<XID, uint64> xa_committed;
    std::set<XID> xa_rollbacked;
    ...
    */
      ...
      switch (ev->get_type_code()) {
        ...
        case binary_log::QUERY_EVENT: {
          if(!strncmp(((Query_log_event*)ev)->query, "XA START", 8)) {
        	in_transaction= true;
			XID xid = get_xid_from_xa_commit_rollback(((Query_log_event *)ev)->query, nullptr);
			if (xa_committed.count(xid)) //in case there are sever same xids in the last binlog
			  xa_committed.erase(xid);
		  }
          if(!strncmp(((Query_log_event *)ev)->query, "XA COMMIT", 9)){
        	assert(in_transaction == false);
			in_transaction = false;
			// the format in binlog is: XA COMMIT X'31',X'32',1
			// find gtrid and bqual from the single-quote is ok
			uint64 cts = 0;
			XID xid = get_xid_from_xa_commit_rollback(((Query_log_event *)ev)->query, &cts);
			xa_committed[xid] = cts;
			if (xa_prepared.count(xid))
			  xa_prepared.erase(xid);
          }
          if(!strncmp(((Query_log_event *)ev)->query, "XA ROLLBACK", 11)){
			assert(in_transaction == false);
			in_transaction = false;

			XID xid = get_xid_from_xa_commit_rollback(((Query_log_event *)ev)->query, nullptr);
			xa_rollbacked.insert(xid);
			if (xa_prepared.count(xid))
			  xa_prepared.erase(xid);
		  }
        ...
        case binary_log::XA_PREPARE_LOG_EVENT: {
		  ...
		  XID xid = xev->get_xid();
		  if (!xev->is_one_phase()) {
			xa_prepared.insert(xid);
			if (xa_committed.count(xid))
			  xa_committed.erase(xid);
		  } else {
			xa_committed[xid] = 0;
			if (xa_prepared.count(xid))
     ...
     */
    res = res || (total_ha_2pc > 1 && ha_recover_ex(&xids, &xa_committed, &xa_rollbacked, &xa_prepared));
  }
