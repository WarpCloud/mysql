...
//    if (trx != nullptr && index->table->is_visible_user_table()) {
//	   row_upd_index_entry_sys_field(new_entry, index, DATA_GTRX_ID, trx->gxid);
//	}
  }
    ...
    dict_index_t *index)      /*!< in: index corresponding to page */
{
  ...
  if (page) {
    ...
    if (!(flags & BTR_KEEP_SYS_FLAG)) {
      ...
      row_upd_rec_sys_fields_in_recovery_ex(
          rec, page_zip,
          rec_get_offsets(rec, index, offsets_, ULINT_UNDEFINED, &heap), pos,
          trx_id, roll_ptr, update_cts, cts);
    ...
    mtr_t *mtr)            /*!< in/out: mini-transaction */
{
  ...
  row_upd_rec_sys_fields_ex(rec, page_zip, index, offsets, trx, roll_ptr, true);

  btr_cur_del_mark_set_clust_rec_log(rec, index, trx->id, trx->cts, roll_ptr, mtr);
