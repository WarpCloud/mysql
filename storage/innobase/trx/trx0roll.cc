                          ...
                          complete rollback */
{
  ...
  trx_sys_update_trx_status(trx, TRX_SYS_STATUS_ROLLBACK);
    ...
    trx_t *trx)           /*!< in: transaction handle */
{
  ...
  trx->in_setting_cts = true;

  cts_node = row_cts_node_create(trx, heap);
  ...
  if (trx_is_rseg_updated(trx)) {
    ...
    thr = pars_complete_graph_for_exec(cts_node, trx, heap, nullptr);
...
//      ut_ad(trx->in_mysql_trx_list);
      //Active user xa transcation may be In TRX_STATE_ACTIVE during recover
      ut_ad((trx->is_recovered && trx->xid && !trx->xid->get_my_xid())
                ...
                TRUE=roll back all non-PREPARED transactions */
{
  ...
  if (trx->xid && !trx->xid->get_my_xid() && trx->xid->get_gtrid_length() > 0) {
     /*All External xa transaction with real xid should be handled on server
      * level. If the gtrid_length is 0, this xa transaction abort before doing
      * xa prepare, so just rollback it.
     */
//     if (state == TRX_STATE_PREPARED)
//       trx->xid->set_is_xa_prepared_during_recover(true);
 	 ib::info() << "Skip handle XA transaction with xid " << trx_get_id_for_print(trx);
     return(FALSE);
  }
...
{
  // #WARP-62733 while in setting cts stage, undo log should not be truncated.
  if (trx->in_setting_cts) {
	  return;
  }
