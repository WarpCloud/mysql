...
//#define GXID_HINT_START "/\\*K! \\s*GRV_UP_ID\\s* ([0-9]*) \\s*GRV_LOW_ID\\s* ([0-9]*) \\s*GRV_IDS\\s* ([0-9|,]*)\\s*\\*/"
...
std::atomic<unsigned long long> global_m_sts{1};

// parse sts from hint like "/*STS! 123456 */"
// STS_TOO_SMALL_ERROR: enable_tso is open and sts is too small
// TSO_IS_CLOSED_ERROR: enable_tso is closed but sts is not null
// PARSE_STS_HINT_RETURN: others
int parse_sts_hint(THD *thd) {
  if (thd == NULL) return PARSE_STS_HINT_RETURN;

  if (thd->query().str != nullptr && thd->query().length > 0) {
    std::string query_str = thd->query().str;
    std::string gxid_hint_key = "/*STS!";

    std::string::size_type gxid_hint_index = query_str.find(gxid_hint_key);
    if (gxid_hint_index != query_str.npos) {
      if (global_m_sts.load() == 0) {
        return TSO_IS_CLOSED_ERROR;
      } else {
        std::string gxid_hint_end_key = "*/";
        std::string::size_type gxid_hint_end_index =
          query_str.find(gxid_hint_end_key, gxid_hint_index);
        std::string cts_str =
          query_str.substr(gxid_hint_index + gxid_hint_key.length() + 1,
                           gxid_hint_end_index - gxid_hint_index -
                             gxid_hint_key.length() - 2);

        if (cts_str.length() > 0) {
          thd->tx_sts = std::strtoull(cts_str.c_str(), nullptr, 10);
          if (uint64_t(thd->tx_sts) < global_m_sts.load()) {
            return STS_TOO_SMALL_ERROR;
          } else {
            return PARSE_STS_HINT_RETURN;
          }
        }
      }
    } else {
...
bool parse_sql(THD *thd, Parser_state *parser_state,
  ...
  */
  ...
  //parse_gxid_hint(thd);
  // when sts is smaller than global oldest sts, sts_check will return true;
  int sts_check = parse_sts_hint(thd);
  bool mysql_parse_status = thd->sql_parser();
  if(!mysql_parse_status) {
    // enable_tso is open and sts is too small
    if (sts_check == TSO_IS_CLOSED_ERROR) {
      thd->lex->sql_command = SQLCOM_END;
      auto error_no = ER_TSO_IS_CLOSED;
      auto error_message = "The switch of innodb_enable_tso is closed, but sts is not null";
      auto error_sqlstate = "45000";
      parser_da->set_error_status(error_no, error_message, error_sqlstate);
      parser_da->push_warning(thd, error_no, error_sqlstate, Sql_condition::SL_ERROR, error_message);
      mysql_parse_status = true;
    }
    // enable_tso is closed but sts is not null
    else if (sts_check == STS_TOO_SMALL_ERROR) {
      thd->lex->sql_command = SQLCOM_END;
      auto error_no = ER_STS_TOO_SMALL;
      auto error_message = "The value of this sts is too small and may query the data that has been purged";
      auto error_sqlstate = "45000";
      parser_da->set_error_status(error_no, error_message, error_sqlstate);
      parser_da->push_warning(thd, error_no, error_sqlstate, Sql_condition::SL_ERROR, error_message);
      mysql_parse_status = true;
