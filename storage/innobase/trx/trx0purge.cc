...
const page_size_t TrxUndoRsegsIterator::set_next() {
  ...
  if (m_iter != m_trx_undo_rsegs.end()) {
    ...
    m_purge_sys->iter.trx_gxid = (*m_iter)->last_trx_cts;
  ...
  m_purge_sys->iter.trx_gxid = m_purge_sys->rseg->last_trx_cts;
    ...
    mtr_t *mtr)         /*!< in: mtr */
{
  ...
  /* Write the trx gxid to the undo log header */
  mlog_write_ull(undo_header + TRX_UNDO_TRX_GXID, trx->cts, mtr);

  /* for compatible*/
  undo->flag |= TRX_UNDO_FLAG_GXID;
                            ...
                            handled */
{
  ...
  purge_sys->iter.trx_gxid = rseg->last_trx_cts + 1;
  ...
  if ((flag & TRX_UNDO_FLAG_GXID) != 0) {
	  trx_gxid = mach_read_from_8(log_hdr + TRX_UNDO_TRX_GXID);
  }
        ...
        mem_heap_t *heap)       /*!< in: memory heap where copied */
{
  ...
  if (srv_enable_tso) {
    if (purge_sys->iter.trx_gxid != 0 && purge_sys->iter.trx_gxid > purge_sys->view.m_sts) {
      return (nullptr);
    }
  }
