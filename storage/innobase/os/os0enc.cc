...
bool Encryption::encrypt_log_block(const IORequest &type, byte *src_ptr,
  ...
  if (opt_encryption_algorithm == SM4) {
      block_size = MY_SM4_BLOCK_SIZE;
  } else {
      block_size = MY_AES_BLOCK_SIZE;
  }
  main_len = (data_len / block_size) * block_size;
    ...
    case AES: {
      ...
      if (opt_encryption_algorithm == SM4) {
          elen = my_sm4_encrypt(
                  ...
                  my_sm4_cbc, reinterpret_cast<unsigned char *>(m_iv), false);

          if (elen == MY_SM4_BAD_DATA) {
              return (false);
          }
      } else {
          elen = my_aes_encrypt(
      ...
      if (remain_len != 0) {
        if (opt_encryption_algorithm == SM4) {
          remain_len = MY_SM4_BLOCK_SIZE * 2;
          ...
          elen =
                  my_sm4_encrypt(dst_ptr + LOG_BLOCK_HDR_SIZE + data_len - remain_len,
                                 ...
                                 static_cast<uint32>(m_klen), my_sm4_cbc,
...
bool Encryption::encrypt_low(byte *src, ulint src_len, byte *dst,
  ...
  switch (m_type) {
    ...
    case AES: {
      ...
      lint elen = 0;
      if (opt_encryption_algorithm == SM4) {
        ut_ad(data_len >= 2 * MY_SM4_BLOCK_SIZE);
        chunk_len = MY_SM4_BLOCK_SIZE;
        remain_len = data_len - chunk_len;

        elen =
            my_sm4_encrypt(src + FIL_PAGE_DATA, static_cast<uint32>(chunk_len),
                           dst + FIL_PAGE_DATA, reinterpret_cast<byte *>(m_key),
                           static_cast<uint32>(m_klen), my_sm4_cbc,
                           reinterpret_cast<byte *>(m_iv), false);

        if (elen == MY_SM4_BLOCK_SIZE) {
      ...
      if (remain_len != 0) {
        ...
        if (opt_encryption_algorithm == SM4) {
          constexpr size_t trailer_len = MY_SM4_BLOCK_SIZE * 2;
          byte buf[trailer_len];

          elen = my_sm4_encrypt(dst + FIL_PAGE_DATA + data_len - trailer_len,
    ...
    case AES: {
      ...
      if (remain_len != 0) {
        ...
        if (opt_encryption_algorithm == SM4) {
          ...
          elen = my_sm4_decrypt(remain_buf, static_cast<uint32>(remain_len),
      ...
      if (opt_encryption_algorithm == SM4) {
        ...
        elen = my_sm4_decrypt(dst, static_cast<uint32>(main_len), ptr,
                              reinterpret_cast<unsigned char *>(m_key),
                              static_cast<uint32>(m_klen), my_sm4_cbc,
                              reinterpret_cast<unsigned char *>(m_iv), false);
        if (elen == MY_AES_BAD_DATA) {
