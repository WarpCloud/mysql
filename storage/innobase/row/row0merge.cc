...
#define dbug_run_purge false
    ...
    if (online) {
      ...
      trx_id_t gtrx_id = 0;
      if (clust_index->table->is_visible_user_table()) {
    	  gtrx_id = row_get_rec_gtrx_id(rec, clust_index, offsets);
      }

      if (!trx->read_view->changes_visible_cts(
              row_get_rec_trx_id(rec, clust_index, offsets), false, gtrx_id, clust_index->table->name, clust_index->table->is_visible_user_table(), nullptr)) {
        rec_t *old_vers;
