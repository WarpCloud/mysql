...
void row_upd_rec_sys_fields_in_recovery_ex(rec_t *rec, page_zip_des_t *page_zip,
                                        const ulint *offsets, ulint pos,
                                        trx_id_t trx_id, roll_ptr_t roll_ptr, bool update_cts, trx_id_t cts) {
  ...
  if (page_zip) {
    ...
    if (update_cts) {
    	trx_write_cts(field + DATA_TRX_ID_LEN + DATA_ROLL_PTR_LEN, cts);
    }
  }
}
...
void row_upd_rec_sys_fields_in_recovery(rec_t *rec, page_zip_des_t *page_zip,
	...
	row_upd_rec_sys_fields_in_recovery_ex(rec, page_zip, offsets, pos, trx_id, roll_ptr, false, 0);
}
...
void row_upd_index_entry_sys_field(dtuple_t *entry, dict_index_t *index,
  ...
  } else if (type == DATA_GTRX_ID) {
//	ut_ad(val > 0);
	trx_write_cts(field, val);
  }
}
...
//	  row_upd_index_entry_sys_field(entry, index, DATA_GTRX_ID, trx->gxid);
//  }
