    ...
    uint32_t flags)    /*!< in: table flags */
{
  switch (error) {
    ...
    case DB_BLOCKING_READ_WAIT_TIMEOUT:
      my_error(ER_BLOCKING_READ_WAIT_TIMEOUT, MYF(0));
      break;
    case DB_INTERRUPTED:
...
static MY_ATTRIBUTE((warn_unused_result)) const ulint *innobase_build_col_map(
  ...
  assert(new_table->get_n_cols() + dict_table_get_n_v_cols(new_table) >=
              altered_table->s->fields + new_table->get_n_sys_cols());
  assert(old_table->get_n_cols() + dict_table_get_n_v_cols(old_table) >=
              table->s->fields + old_table->get_n_sys_cols());
  assert(!!add_cols == !!(ha_alter_info->handler_flags &
  ...
  for (uint old_i = 0; old_i + old_table->get_n_sys_cols() < old_table->n_cols; old_i++) {
  ...
  if (i + old_table->get_n_sys_cols() < old_table->n_cols) {
    /* There should be exactly one extra field,
    the FTS_DOC_ID. */
	assert(DICT_TF2_FLAG_IS_SET(old_table, DICT_TF2_FTS_HAS_DOC_ID));
    assert(i + old_table->get_n_sys_cols() + 1 == old_table->n_cols);
    assert(!strcmp(old_table->get_col_name(i), FTS_DOC_ID_COL_NAME));
    if (altered_table->s->fields + new_table->get_n_sys_cols() - new_table->n_v_cols <
        new_table->n_cols) {
      assert(DICT_TF2_FLAG_IS_SET(new_table, DICT_TF2_FTS_HAS_DOC_ID));
      assert(altered_table->s->fields + new_table->get_n_sys_cols() + 1 ==
  ...
  for (; i < old_table->n_cols; i++) {
	// deal with the condition : add system column gxid
	if (new_table->is_visible_user_table() && !old_table->is_visible_user_table()) {
		col_map[i] = i + new_table->n_cols -1 - old_table->n_cols;
	} else {
...
bool ha_innobase::prepare_inplace_alter_table_impl(
  ...
  if (new_dd_tab->options().exists("visible_user_table")) {
