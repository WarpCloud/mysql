...
static trx_id_t compute_oldest_sts_with_time_window(trx_id_t cur_oldest_sts) {
	uint64 time_window = ((uint64)(srv_purge_time_window * 1000 + 1)) << srv_logical_time_size;
	return cur_oldest_sts <= time_window ? 1: cur_oldest_sts - time_window;
}


void MVCC::set_oldest_sts(ReadView *oldest_sts_view) {
  ReadView *view;
  trx_id_t oldest_sts = 0;

  mutex_enter(&trx_sys->mutex);
  for (view = UT_LIST_GET_LAST(m_views); view != nullptr;
         view = UT_LIST_GET_PREV(m_view_list, view)) {
	if (!view->is_closed() && view->m_sts != 0) {
		if (oldest_sts == 0) {
			oldest_sts = view->m_sts;
		} else if (view->m_sts < oldest_sts) {
			oldest_sts = view->m_sts;
		}
	}
  }
  ...
  if (oldest_sts == 0) {
	oldest_sts = trx_sys_get_max_ts() + 1;
  }

  // set time window t1 that is allowed to delay to purge
  if (oldest_sts != TRX_SYS_MAX_TS_INIT + 1) {
	  oldest_sts = compute_oldest_sts_with_time_window(oldest_sts);
  }
  if (srv_enable_tso) {
    global_m_sts.store(oldest_sts);
  } else {
    global_m_sts.store(0);
  }


  oldest_sts_view->m_sts = oldest_sts;
}
...
void MVCC::view_open(ReadView *&view, trx_t *trx) {
  ...
  if (trx->mysql_thd != nullptr && trx->mysql_thd->tx_sts > 0) {
	 trx_sys_update_max_ts(trx->mysql_thd->tx_sts);
  }
  ...
  if (view != nullptr) {
    ...
    if (trx_is_autocommit_non_locking(trx) && view->empty()) {
      ...
      if (view->m_low_limit_id == trx_sys_get_max_trx_id()) {
		 ...
		 if (thd != nullptr && !trx->ddl_operation) {
			 view->m_sts = thd->tx_sts;
		 }
        return;
      } else {
        view->m_closed = true;
...
/** Check whether the changes by id and cts are visible.
  @param[in]	id	     transaction id to check against the view
  @param[in]	is_history	 whether the row value is history version
  @param[in]	cts	     commit timestamp to check against the view
  @param[in]	name	table name
  @param[in,out]	*error_code  0: success, 1: can not check now, should wait
  @return whether the view sees the modifications of id. */
 bool ReadView::changes_visible_cts(trx_id_t id,
			bool is_history,
			trx_id_t cts,
		    const table_name_t &name,
			bool is_user_visible_table,
	  ...
	  if (m_sts == 0 || !is_user_visible_table) {
	  ...
	  if (cts == 0) {
		  ...
		  trx_id_t status_or_cts = 0;
		  trx_id_t sts = 0;
		  bool found = trx_sys_get_trx_status(id, &status_or_cts, &sts);
		  if(!found) {
			  if (error_code != nullptr) {
				  sql_print_error("trx %llu is not found in status cache", id);
				  *error_code = 2;
			  }
			  return false;
		  }
		  ...
		  case TRX_SYS_STATUS_NONE:
			  // would not come here except a bug, will lost global consistent
			  sql_print_error("read global consistent losts. hit a bug: reading a row whose cts=0 but the status of trx %llu is not in status cache", id);
			  return changes_visible(id, name);
		  ...
		  {
			  // can not wait trx commit here if would block releasing the btree leaf latch
			  if (error_code != nullptr) {
				  *error_code = 1;
				  return false;
			  }

			  // if error_code is not provided, global consistent is not guaranteed
			  sql_print_warning("read global consistent losts.maybe hit a bug: error_code is nullptr, please see func changes_visble_cts");
			  return changes_visible(id, name);
		  }
		  default:
			  cts = status_or_cts;
...
void MVCC::set_view_creator_trx_id(ReadView *view, trx_id_t id, trx_id_t gxid) {
