...
/** Reads the DB_GTRX_ID of a clustered index record.
 @return the value of DB_TRX_ID */
trx_id_t rec_get_gtrx_id(const rec_t *rec,          /*!< in: record */
                        const dict_index_t *index) /*!< in: clustered index */
{
  if (!index->table->is_visible_user_table()) {
	  return 0;
  }
  ulint gtrx_id_col = index->get_sys_col_pos(DATA_GTRX_ID);
  const byte *gtrx_id;
  ulint len;
  ...
  ut_ad(index->table->is_visible_user_table());
  ut_ad(gtrx_id_col > 0);
  ut_ad(gtrx_id_col != ULINT_UNDEFINED);
  ...
  offsets = rec_get_offsets(rec, index, offsets, gtrx_id_col + 1, &heap);

  gtrx_id = rec_get_nth_field(rec, offsets, gtrx_id_col, &len);

  ut_ad(len == DATA_GTRX_ID_LEN);

  if (heap) {
    mem_heap_free(heap);
  }

  return (trx_read_cts(gtrx_id));
}
